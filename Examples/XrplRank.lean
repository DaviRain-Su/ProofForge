...
/-!
Local 2.6.1 allowance take with pause and freeze gates, then cashSelf.
A `grant` writes `allw=5`. Wallet B `takeB` cuts A's `allw`+`bal`
onto B. B `cashSelf` burns 5 and emits 192 drops. Ungranted take
returns 1. Paused take/cash returns 4. Frozen take/cash returns 5.
Anyone else take → 3.
wasm v0 rejects `&&` / `||`; `!cond` extracts. Not Rust `?`.
Zero wasm params. Public still host -22 and tefBAD_AUTH.
Not Sdk.Payments, not a Map.
-/
namespace Examples.XrplRank
