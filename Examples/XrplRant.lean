...
/-!
Local 2.6.1 pause+freeze take, minter claw, then cashSelf.
A `grant` writes `allw=5`. Wallet B `takeB` cuts A's `allw`+`bal`
onto B. Minter `clawB` burns B's 5 and cuts `supp`. A `cashSelf`
burns 5 and emits 192 drops. Ungranted take returns 1. Paused
take/claw/cash returns 4. Frozen take/cash returns 5. Anyone
else claw → 3.
wasm v0 rejects `&&` / `||`; `!cond` extracts. Not Rust `?`.
Zero wasm params. Public still host -22 and tefBAD_AUTH.
Not Sdk.Payments, not a Map.
-/
namespace Examples.XrplRant
...
/-- `{ bal := s.bal }` would copy the previous persist Owner's card. -/
@[pf_entry]
def pause (_s : State) : Except Error (State × UInt64) :=
...
@[pf_entry]
def unpause (_s : State) : Except Error (State × UInt64) :=
...
No grant → 1. Paused → 4. Frozen → 5. Anyone else → 3. -/
@[pf_entry]
...
/-- Minter claws 5 from wallet B and cuts contract `supp`. Paused → 4. -/
@[pf_entry]
...
/-- Caller burns 5, cuts `supp`, emits 192 drops. Paused → 4. Frozen → 5. -/
@[pf_entry]
