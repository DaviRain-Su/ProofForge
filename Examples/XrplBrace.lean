...
/-!
Local 2.6.1 freeze-gated dual timelock via SDK Card.addSelfEsc /
setSelfDueAhead / subSelfEsc / subSelfSupp. A `lockIn` moves 5 onto
contract `esc` with `due = ledgerSqn + 5` (freeze/unfreeze consume
ledgers). Frozen A `cancel` returns 5. After due only wallet B `cashB`
(burns esc+supp, 192 drops to B). Frozen B cash also returns 5.
