...
differ from `Examples.TransientOrderTape` (3-limb records, clear-on-full policy) so both can be
registered independently by the coordinator.
-/

namespace Examples.TransientLedger

open ProofForge.Svm.Sdk

/-- Minimal managed state, mirroring `Examples.MemoryOps` so this module stays inside the standard
