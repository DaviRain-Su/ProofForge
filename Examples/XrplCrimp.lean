...
/-!
Local 2.6.1 mint-cap plus operator pause dual timelock.
A `setCap10` writes `cap=10`; a later `credit` overflows. A `grantOp`
writes `allw=1` so wallet B may `pause`. `lockIn` moves 5 onto contract
`esc` with `due = ledgerSqn + 2`. After due only wallet B `cashB`.
wasm v0 rejects `&&` / `||`; `!cond` extracts. Not Rust `?`.
Zero wasm params. Public still host -22 and tefBAD_AUTH.
Not Sdk.Payments, not a Map.
-/
namespace Examples.XrplCrimp
