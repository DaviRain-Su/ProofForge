...
/-!
Local 2.6.1 pause+freeze dual timelock with mint cap.
A `setCap10` writes `cap=10` on the minter card. A later `credit`
past that returns overflow. `lockIn` moves 5 onto contract `esc`
with `due = ledgerSqn + 5`. Paused lockIn returns 4. Frozen
cancel/cashB return 5. After due only wallet B `cashB`.
wasm v0 rejects `&&` / `||`; `!cond` extracts. Not Rust `?`.
Zero wasm params. Public still host -22 and tefBAD_AUTH.
Not Sdk.Payments, not a Map.
-/
namespace Examples.XrplCleft
...
def credit (s : State) : Except Error (State × UInt64) :=
  if !Pausable.isRunning (Context.peekHaltLit "b5f762798a53d543a014caf8b297cff8f2f937e8") then
    ...
    .error .overflow
  else if !Card.selfSuppFitsCapLit "b5f762798a53d543a014caf8b297cff8f2f937e8" (5 : UInt64) then
    .error .overflow
...
/-- Minter writes `cap=10`. Later credit past that overflows. -/
@[pf_entry]
def setCap10 (s : State) : Except Error (State × UInt64) :=
  if !Access.requireOwner minter then
    .error .unauthorized
  else if !Card.flushCapLit "b5f762798a53d543a014caf8b297cff8f2f937e8" (10 : UInt64) then
    .error .overflow
  else if !Gate.ok Card.restoreCaller then
    .error .overflow
  else if !(s.bal ≤ Gate.u64Max) then
