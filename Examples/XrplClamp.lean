...
/-!
Local 2.6.1 pause+freeze dual timelock via SDK Card helpers.
A `lockIn` moves 5 onto contract `esc` with `due = ledgerSqn + 5`.
Paused lockIn/cancel/cashB return 4. Frozen cancel/cashB return 5.
After due only wallet B `cashB` (burns esc+supp, 192 drops to B).
