...
/-!
Local 2.6.1 freeze-gated send/claw plus caller cashSelf.
A `sendToB` moves 5 onto wallet B; minter `clawB` takes it back and
cuts `supp`. Caller `cashSelf` burns 5 and emits 192 drops. Frozen
send/cash returns 5. B cashSelf is allowed (burns B's own points).
wasm v0 rejects `&&` / `||`; `!cond` extracts. Not Rust `?`.
Zero wasm params. Public still host -22 and tefBAD_AUTH.
Not Sdk.Payments, not a Map.
-/
namespace Examples.XrplReed
...
/-- Caller burns 5 of own points, cuts `supp`, emits 192 drops.
`subSelfSupp` rewrites persist Owner; flush caller `bal` first. Frozen → 5. -/
@[pf_entry]
