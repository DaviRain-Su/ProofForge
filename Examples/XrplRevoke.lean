...
/-!
Local 2.6.1 allowance revoke plus timelock. A `grant` writes `allw=5`.
A `revoke` writes `allw=0` so later B `takeAndLock` overflows. After a
live grant, B `takeAndLock` moves 5 onto contract `esc` with
`due = ledgerSqn + 2`. A cancel before due; after due only B `cashB`.
wasm v0 rejects `&&` / `||`; `!cond` extracts. Not Rust `?`.
Zero wasm params. Public still host -22 and tefBAD_AUTH.
Not Sdk.Payments, not a Map.
-/
namespace Examples.XrplRevoke
...
/-- Caller writes `allw=0` on this card. Later takeAndLock overflows. -/
@[pf_entry]
def revoke (s : State) : Except Error (State × UInt64) :=
  if !Pausable.isRunning (Context.peekHaltLit "b5f762798a53d543a014caf8b297cff8f2f937e8") then
    .error .paused
  else if !Gate.ok Card.restoreCaller then
    .error .overflow
  else if !Gate.ok (Card.flushCallerAllw (0 : UInt64)) then
    .error .overflow
  else if !(s.bal ≤ Gate.u64Max) then
    .error .overflow
  else
...
No grant / revoked allw → 1. Anyone else → 3. -/
@[pf_entry]
