...
XRPL（`--target xrpl`；`wasm` 会被拒绝并提示选具体链）写出 `Name.rs`
（XRPL Bedrock / `xrpl_wasm_std` 方言；zero-tool，`deployable=false`；
工程门 `runtime-tests/xrpl/check.sh` 用本地 stub crate 做 cargo check）。
