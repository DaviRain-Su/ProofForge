...
`PhoenixV1Profile` 当前在 128-seat 固定账户内支持前四个 distinct trader 的 exact
Sokoban insertion，包括第三次的 LL/LR/RR/RL rotation 和第四次的 red-uncle recolor；
持久状态只保存 one-based slot index，不使用 heap `Map` 或账户外 pointer。
