...
/// Tag 10 withdraw accepts at most five ids (`len ∈ {0..=5}`; max wire 90 bytes).
...
fn official_raw_cancel_by_id_rejects_noncanonical_wire_and_invalid_side_atomically() {
    ...
    let before = market.data.clone();
    for data in [
        // length=6 exceeds profile capacity 5 / maxDataLen 90 for tag 10.
        {
            let mut over = vec![10, 6, 0, 0, 0];
            for i in 1u64..=6u64 {
                over.push(0u8);
