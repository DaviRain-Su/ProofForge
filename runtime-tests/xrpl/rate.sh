...
if [[ -z "$python" ]]; then
  echo "xrpl-rate: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-rate: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-rate: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-rate: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-rate: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-rate: $RPC $info" >&2
...
echo "xrpl-rate: restarted $info" >&2

echo "xrpl-rate: building XrplRate.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplRate
wasm="$root/build/xrpl/XrplRate.wasm"
...
want "$(call_fn cash384)" 0 cash384
[[ "$(slot "$OWNER" bal)" == "0" ]] || { echo "FAIL: after cash384 A want 0" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "0" ]] || { echo "FAIL: after cash384 supp want 0" >&2; exit 1; }
printf '{"rpc_url":"%s","account":"%s"}\n' "$RPC" "$OWNER" >"$cfg"
...
[[ "$got" == "384" ]] || { echo "FAIL: A XRP net-of-fee want 384 got $got (before=$before after=$after)" >&2; exit 1; }

rm -f "$cfg"
echo "xrpl-rate: ok contract=$contract cash384 +384-fee (not Sdk.Payments)"
