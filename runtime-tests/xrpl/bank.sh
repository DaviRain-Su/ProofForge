...
if [[ -z "$python" ]]; then
  echo "xrpl-bank: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-bank: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-bank: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-bank: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-bank: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-bank: $RPC $info" >&2
...
echo "xrpl-bank: restarted $info" >&2

echo "xrpl-bank: building XrplBank.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplBank
wasm="$root/build/xrpl/XrplBank.wasm"
...
echo "xrpl-bank: ok contract=$contract pause=4 then cash (bal=0 supp=0 emit 192; not Sdk.Payments)"
