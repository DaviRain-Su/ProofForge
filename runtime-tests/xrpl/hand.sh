#!/usr/bin/env bash
# Live AlphaNet: two-step Ownable across wallets.
# Wallet B deploys + propose(genesis); genesis accept + bump.
set -euo pipefail
...
if [[ -z "$python" ]]; then
  echo "xrpl-alphanet-hand: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-alphanet-hand: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-alphanet-hand: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$nid" != "21337" || "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-alphanet-hand: skip: not AlphaNet SmartContract ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-alphanet-hand: $RPC $info" >&2

wasm="$root/build/xrpl-alphanet/XrplHand.wasm"
if [[ ! -f "$wasm" ]]; then
  lake exe pf -- build --target xrpl-alphanet --out "$root/build/xrpl-alphanet" XrplHand
fi
...
propose_out="$(call_fn "$WALLET_B" propose)"
echo "$propose_out" >&2
"$python" -I -S -c 'import json,sys; d=json.load(sys.stdin); assert d.get("result")=="tesSUCCESS" and d.get("vmReturnCode")==0, d' <<<"$propose_out"

denied="$(call_fn "$WALLET_A" bump || true)"
echo "$denied" >&2
"$python" -I -S -c 'import json,sys; d=json.load(sys.stdin); assert d.get("vmReturnCode")==3, d' <<<"$denied"

accept_out="$(call_fn "$WALLET_A" accept)"
echo "$accept_out" >&2
"$python" -I -S -c 'import json,sys; d=json.load(sys.stdin); assert d.get("result")=="tesSUCCESS" and d.get("vmReturnCode")==0, d' <<<"$accept_out"
...
rm -f "$cfg"
[[ "$value" == "1" ]] || { echo "FAIL: expected genesis card value=1, got $value" >&2; exit 1; }

echo "xrpl-alphanet-hand: ok contract=$contract A.value=$value (cross-wallet accept)"
