#!/usr/bin/env bash
# Local 2.6.1: Gate.and2/ok escrow. credit 10; latch A=5 esc=5; unlatch A=10 esc=0.
...
if [[ -z "$python" ]]; then
  echo "xrpl-latch: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-latch: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-latch: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-latch: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-latch: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-latch: $RPC $info" >&2
...
echo "xrpl-latch: restarted $info" >&2

echo "xrpl-latch: building XrplLatch.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplLatch
wasm="$root/build/xrpl/XrplLatch.wasm"
