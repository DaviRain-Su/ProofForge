#!/usr/bin/env bash
# Local 2.6.1: Pay.emitToCaller Payment 192 drops to the caller.
...
if [[ -z "$python" ]]; then
  echo "xrpl-emit: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-emit: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-emit: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-emit: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-emit: skip: public AlphaNet still tefBAD_AUTH -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-emit: $RPC $info" >&2
...
echo "xrpl-emit: restarted $info" >&2

echo "xrpl-emit: building XrplEmit.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplEmit
wasm="$root/build/xrpl/XrplEmit.wasm"
...
rm -f "$cfg"
echo "xrpl-emit: ok contract=$contract ping tesSUCCESS/0 (local emit_built_txn; not Sdk.Payments)"
