...
if [[ -z "$python" ]]; then
  echo "xrpl-brace: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-brace: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-brace: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-brace: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-brace: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-brace: $RPC $info" >&2
...
echo "xrpl-brace: restarted $info" >&2

echo "xrpl-brace: building XrplBrace.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplBrace
wasm="$root/build/xrpl/XrplBrace.wasm"
...
echo "xrpl-brace: ok contract=$contract freeze cancel=5 then B cashB +192-fee (not Sdk.Payments)"
