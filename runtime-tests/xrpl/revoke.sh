...
if [[ -z "$python" ]]; then
  echo "xrpl-revoke: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-revoke: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-revoke: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-revoke: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-revoke: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-revoke: $RPC $info" >&2
...
echo "xrpl-revoke: restarted $info" >&2

echo "xrpl-revoke: building XrplRevoke.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplRevoke
wasm="$root/build/xrpl/XrplRevoke.wasm"
...
want "$(call_fn takeAndLock "$WALLET_B")" 1 no-grant
[[ "$(slot "$OWNER" bal)" == "10" ]] || { echo "FAIL: no-grant must keep A=10" >&2; exit 1; }

want "$(call_fn grant)" 0 grant-then-revoke
want "$(call_fn revoke)" 0 revoke
[[ "$(slot "$OWNER" allw)" == "0" ]] || { echo "FAIL: after revoke allw want 0" >&2; exit 1; }
want "$(call_fn takeAndLock "$WALLET_B")" 1 after-revoke
[[ "$(slot "$OWNER" bal)" == "10" ]] || { echo "FAIL: revoke must keep A=10" >&2; exit 1; }

want "$(call_fn grant)" 0 grant
want "$(call_fn takeAndLock "$WALLET_B")" 0 takeAndLock
[[ "$(slot "$OWNER" bal)" == "5" ]] || { echo "FAIL: after takeAndLock A want 5" >&2; exit 1; }
[[ "$(slot "$OWNER" allw)" == "0" ]] || { echo "FAIL: after takeAndLock allw want 0" >&2; exit 1; }
[[ "$(slot "$contract" esc)" == "5" ]] || { echo "FAIL: after takeAndLock esc want 5" >&2; exit 1; }
...
want "$(call_fn grant)" 0 grant2
want "$(call_fn takeAndLock "$WALLET_B")" 0 takeAndLock2
[[ "$(slot "$OWNER" bal)" == "5" ]] || { echo "FAIL: after takeAndLock2 A want 5" >&2; exit 1; }
...
echo "xrpl-revoke: ok contract=$contract freeze cancel=5 then B cashB +192-fee (not Sdk.Payments)"
