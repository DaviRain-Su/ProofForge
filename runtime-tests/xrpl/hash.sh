#!/usr/bin/env bash
# Engineering local-node gate for XrplHash SHA-512Half literal.
# Missing docker/bedrock → skip (exit 0). Deploys ProofForge XrplHash.wasm only.
# stamp writes SHA-512Half("vault") first little-endian u64 into hashed.
set -euo pipefail
...
xrpl_init xrpl-local-hash

wasm="$root/build/xrpl/XrplHash.wasm"
fixture="$here/fixture"
staged="$fixture/contract/target/wasm32-unknown-unknown/release/Counter.wasm"
# SHA-512("vault")[:32] first 8 bytes little-endian.
want="4898221643817197762"

if [[ ! -f "$wasm" ]]; then
  echo "xrpl-local-hash: building XrplHash.wasm" >&2
...
echo "xrpl-local-hash: starting local node" >&2
(cd "$fixture" && "$bedrock" node stop >/dev/null 2>&1) || true
...
xrpl_require_equal "$(xrpl_slot_u64 "$owner" "$contract" "hashed")" "$want" \
  "hashed SHA-512Half(vault) lo64"

echo "xrpl-local-hash: ok (hashed=$want; engineering only)"
