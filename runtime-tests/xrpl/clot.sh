...
if [[ -z "$python" ]]; then
  echo "xrpl-clot: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-clot: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-clot: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-clot: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-clot: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-clot: $RPC $info" >&2
...
echo "xrpl-clot: restarted $info" >&2

echo "xrpl-clot: building XrplClot.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplClot
wasm="$root/build/xrpl/XrplClot.wasm"
...
want "$(call_fn credit)" 0 credit
want "$(call_fn credit)" 0 credit
[[ "$(slot "$OWNER" bal)" == "20" ]] || { echo "FAIL: A bal want 20" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "20" ]] || { echo "FAIL: supp want 20" >&2; exit 1; }

want "$(call_fn sendToB)" 0 sendToB
[[ "$(slot "$OWNER" bal)" == "15" ]] || { echo "FAIL: after sendToB A want 15" >&2; exit 1; }
[[ "$(slot "$OWNER_B" bal)" == "5" ]] || { echo "FAIL: after sendToB B want 5" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "20" ]] || { echo "FAIL: sendToB must keep supp=20" >&2; exit 1; }
...
[[ "$(slot "$contract" supp)" == "15" ]] || { echo "FAIL: after clawB supp want 15" >&2; exit 1; }
[[ "$(slot "$OWNER" bal)" == "15" ]] || { echo "FAIL: clawB must keep A=15" >&2; exit 1; }

want "$(call_fn freeze)" 0 freeze-burn
want "$(call_fn burn)" 5 frozen-burn
[[ "$(slot "$OWNER" bal)" == "15" ]] || { echo "FAIL: freeze burn must keep A=15" >&2; exit 1; }
want "$(call_fn unfreeze)" 0 unfreeze-burn
want "$(call_fn burn)" 0 burn
[[ "$(slot "$OWNER" bal)" == "10" ]] || { echo "FAIL: after burn A want 10" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "10" ]] || { echo "FAIL: after burn supp want 10" >&2; exit 1; }
...
echo "xrpl-clot: ok contract=$contract freeze cancel=5 then B cashB +192-fee (not Sdk.Payments)"
