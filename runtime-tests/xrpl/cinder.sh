...
if [[ -z "$python" ]]; then
  echo "xrpl-cinder: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-cinder: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-cinder: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-cinder: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-cinder: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-cinder: $RPC $info" >&2
...
echo "xrpl-cinder: restarted $info" >&2

echo "xrpl-cinder: building XrplCinder.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplCinder
wasm="$root/build/xrpl/XrplCinder.wasm"
...
want "$(call_fn burnFrom "$WALLET_B")" 1 no-grant
[[ "$(slot "$OWNER" bal)" == "15" ]] || { echo "FAIL: no-grant must keep A=15" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "15" ]] || { echo "FAIL: no-grant must keep supp=15" >&2; exit 1; }

want "$(call_fn grant)" 0 grant
want "$(call_fn burnFrom "$WALLET_B")" 0 burnFrom
[[ "$(slot "$OWNER" bal)" == "10" ]] || { echo "FAIL: after burnFrom A want 10" >&2; exit 1; }
[[ "$(slot "$OWNER" allw)" == "0" ]] || { echo "FAIL: after burnFrom allw want 0" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "10" ]] || { echo "FAIL: after burnFrom supp want 10" >&2; exit 1; }
b_bal="$(slot "$OWNER_B" bal)"
[[ "$b_bal" == "0" || "$b_bal" == "missing" ]] || { echo "FAIL: burnFrom must not credit B got $b_bal" >&2; exit 1; }
...
echo "xrpl-cinder: ok contract=$contract freeze cancel=5 then B cashB +192-fee (not Sdk.Payments)"
