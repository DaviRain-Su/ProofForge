...
if [[ -z "$python" ]]; then
  echo "xrpl-crimp: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-crimp: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-crimp: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-crimp: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-crimp: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-crimp: $RPC $info" >&2
...
echo "xrpl-crimp: restarted $info" >&2

echo "xrpl-crimp: building XrplCrimp.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplCrimp
wasm="$root/build/xrpl/XrplCrimp.wasm"
...
want "$(call_fn setCap10)" 0 setCap10
want "$(call_fn credit)" 1 over-cap
[[ "$(slot "$OWNER" bal)" == "10" ]] || { echo "FAIL: over-cap must keep A=10" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "10" ]] || { echo "FAIL: over-cap must keep supp=10" >&2; exit 1; }
...
echo "xrpl-crimp: ok contract=$contract freeze cancel=5 then B cashB +192-fee (not Sdk.Payments)"
