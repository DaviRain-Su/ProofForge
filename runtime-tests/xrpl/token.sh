#!/usr/bin/env bash
# Local 2.6.1: treasury plus pause/freeze on one 11-export program.
...
if [[ -z "$python" ]]; then
  echo "xrpl-token: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-token: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-token: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-token: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-token: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-token: $RPC $info" >&2
...
echo "xrpl-token: restarted $info" >&2

echo "xrpl-token: building XrplToken.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplToken
wasm="$root/build/xrpl/XrplToken.wasm"
...
want "$(call_fn credit)" 4 paused-credit
...
echo "xrpl-token: ok contract=$contract pause/freeze + clawB/burn/cashSelf (not Sdk.Payments)"
