#!/usr/bin/env bash
# Live AlphaNet: per-user freeze on caller cards.
...
if [[ -z "$python" ]]; then
  echo "xrpl-alphanet-lock: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-alphanet-lock: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-alphanet-lock: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$nid" != "21337" || "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-alphanet-lock: skip: not AlphaNet SmartContract ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-alphanet-lock: $RPC $info" >&2

echo "xrpl-alphanet-lock: building XrplLock.wasm" >&2
lake exe pf -- build --target xrpl-alphanet --out "$root/build/xrpl-alphanet" XrplLock
wasm="$root/build/xrpl-alphanet/XrplLock.wasm"
...
echo "xrpl-alphanet-lock: ok contract=$contract A.bal=1 B.bal=4 freeze=status5"
