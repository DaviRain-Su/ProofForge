#!/usr/bin/env bash
# Local 2.6.1: lockIn writes due=ledgerSqn+2; only wallet B cashB after due
# burns esc+supp and emits 192 drops to B.
...
if [[ -z "$python" ]]; then
  echo "xrpl-payout: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-payout: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-payout: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-payout: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-payout: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-payout: $RPC $info" >&2
...
echo "xrpl-payout: restarted $info" >&2

echo "xrpl-payout: building XrplPayout.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplPayout
wasm="$root/build/xrpl/XrplPayout.wasm"
...
# due = sqn+2, so the next Call is still early.
want "$(call_fn cashB "$WALLET_B")" 1 too-early
[[ "$(slot "$contract" esc)" == "5" ]] || { echo "FAIL: early cash must keep esc=5" >&2; exit 1; }
...
echo "xrpl-payout: ok contract=$contract lockIn then B cashB +192-fee (not Sdk.Payments)"
