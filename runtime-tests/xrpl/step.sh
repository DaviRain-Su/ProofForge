#!/usr/bin/env bash
# Live AlphaNet: two-step Ownable, same-wallet self-nominate.
# initialize → bump (value=1) → propose → accept → bump (value=2).
set -euo pipefail
...
if [[ -z "$python" ]]; then
  echo "xrpl-alphanet-step: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-alphanet-step: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-alphanet-step: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$nid" != "21337" || "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-alphanet-step: skip: not AlphaNet SmartContract ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-alphanet-step: $RPC $info" >&2

wasm="$root/build/xrpl-alphanet/XrplStep.wasm"
if [[ ! -f "$wasm" ]]; then
  lake exe pf -- build --target xrpl-alphanet --out "$root/build/xrpl-alphanet" XrplStep
fi
...
value="$(slot_of value)"
rm -f "$cfg"
[[ "$value" == "2" ]] || { echo "FAIL: expected value=2 after self-accept bump, got $value" >&2; exit 1; }

echo "xrpl-alphanet-step: ok contract=$contract value=$value (self-nominate)"
