#!/usr/bin/env bash
# Local 2.6.1: caller grant allw=5; wallet B takeB moves 5 from A onto B.
...
if [[ -z "$python" ]]; then
  echo "xrpl-take: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-take: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-take: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-take: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-take: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-take: $RPC $info" >&2
...
echo "xrpl-take: restarted $info" >&2

echo "xrpl-take: building XrplTake.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplTake
wasm="$root/build/xrpl/XrplTake.wasm"
...
# B cannot take before grant.
want "$(call_fn takeB "$WALLET_B")" 1 take-no-allw
...
[[ "$(slot "$OWNER" allw)" == "5" ]] || { echo "FAIL: A allw want 5" >&2; exit 1; }

want "$(call_fn takeB "$WALLET_B")" 0 takeB
[[ "$(slot "$OWNER" bal)" == "5" ]] || { echo "FAIL: after take A want 5" >&2; exit 1; }
[[ "$(slot "$OWNER_B" bal)" == "5" ]] || { echo "FAIL: after take B want 5" >&2; exit 1; }
[[ "$(slot "$OWNER" allw)" == "0" ]] || { echo "FAIL: after take A allw want 0" >&2; exit 1; }

want "$(call_fn takeB "$WALLET_B")" 1 take-spent
[[ "$(slot "$OWNER" bal)" == "5" ]] || { echo "FAIL: spent allw must keep A=5" >&2; exit 1; }

rm -f "$cfg"
echo "xrpl-take: ok contract=$contract grant/takeB A=5 B=5 allw=0 (not Sdk.Payments)"
