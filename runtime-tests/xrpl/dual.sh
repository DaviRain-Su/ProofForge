#!/usr/bin/env bash
# Local 2.6.1: A cancel before due; after due only wallet B claimB.
...
if [[ -z "$python" ]]; then
  echo "xrpl-dual: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-dual: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-dual: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-dual: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-dual: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-dual: $RPC $info" >&2
...
echo "xrpl-dual: restarted $info" >&2

echo "xrpl-dual: building XrplDual.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplDual
wasm="$root/build/xrpl/XrplDual.wasm"
...
[[ "$(slot "$contract" esc)" == "5" ]] || { echo "FAIL: early B claim must keep esc=5" >&2; exit 1; }
...
rm -f "$cfg"
echo "xrpl-dual: ok contract=$contract A cancel then B claimB after due (not Sdk.Payments)"
