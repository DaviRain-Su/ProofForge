#!/usr/bin/env bash
# Local 2.6.1: allowance takeB with pause+freeze gates, then B cashSelf.
# Paused take/cash return 4. Frozen take/cash return 5.
...
if [[ -z "$python" ]]; then
  echo "xrpl-rank: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-rank: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-rank: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-rank: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-rank: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-rank: $RPC $info" >&2
...
echo "xrpl-rank: restarted $info" >&2

echo "xrpl-rank: building XrplRank.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplRank
wasm="$root/build/xrpl/XrplRank.wasm"
...
want "$(call_fn pause)" 0 pause-cash
want "$(call_fn cashSelf "$WALLET_B")" 4 paused-cash
[[ "$(slot "$OWNER_B" bal)" == "5" ]] || { echo "FAIL: pause cash must keep B=5" >&2; exit 1; }
want "$(call_fn unpause)" 0 unpause-cash

want "$(call_fn freeze "$WALLET_B")" 0 freeze-cash
want "$(call_fn cashSelf "$WALLET_B")" 5 frozen-cash
[[ "$(slot "$OWNER_B" bal)" == "5" ]] || { echo "FAIL: freeze cash must keep B=5" >&2; exit 1; }
want "$(call_fn unfreeze "$WALLET_B")" 0 unfreeze-cash

printf '{"rpc_url":"%s","account":"%s"}\n' "$RPC" "$OWNER_B" >"$cfg"
before="$(node "$here/alphanet-rpc.js" balance "$cfg")"
want "$(call_fn cashSelf "$WALLET_B")" 0 cashSelf
[[ "$(slot "$OWNER_B" bal)" == "0" ]] || { echo "FAIL: after cashSelf B want 0" >&2; exit 1; }
[[ "$(slot "$contract" supp)" == "5" ]] || { echo "FAIL: after cashSelf supp want 5" >&2; exit 1; }
[[ "$(slot "$OWNER" bal)" == "5" ]] || { echo "FAIL: cashSelf must keep A=5" >&2; exit 1; }
printf '{"rpc_url":"%s","account":"%s"}\n' "$RPC" "$OWNER_B" >"$cfg"
...
rm -f "$cfg"
echo "xrpl-rank: ok contract=$contract takeB pause+freeze then B cashSelf +192-fee (not Sdk.Payments)"
