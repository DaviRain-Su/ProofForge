#!/usr/bin/env bash
# Local 2.6.1: allowance takeB then pause-gated B cashSelf.
# Paused take/cash return 4. Funded first-install Create.
# Public AlphaNet pokeSelf is still -22.
...
if [[ -z "$python" ]]; then
  echo "xrpl-ramp: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-ramp: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-ramp: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-ramp: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-ramp: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-ramp: $RPC $info" >&2
...
echo "xrpl-ramp: restarted $info" >&2

echo "xrpl-ramp: building XrplRamp.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplRamp
wasm="$root/build/xrpl/XrplRamp.wasm"
...
rm -f "$cfg"
echo "xrpl-ramp: ok contract=$contract takeB then pause-gated B cashSelf +192-fee (not Sdk.Payments)"
