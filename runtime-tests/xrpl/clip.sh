...
if [[ -z "$python" ]]; then
  echo "xrpl-clip: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-clip: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-clip: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-clip: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-clip: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-clip: $RPC $info" >&2
...
echo "xrpl-clip: restarted $info" >&2

echo "xrpl-clip: building XrplClip.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplClip
wasm="$root/build/xrpl/XrplClip.wasm"
...
echo "xrpl-clip: ok contract=$contract freeze cancel=5 then B cashB +192-fee (not Sdk.Payments)"
