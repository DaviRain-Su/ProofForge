#!/usr/bin/env bash
# Live AlphaNet gate for XrplVec compile-time slots via parameterized setAt.
# setAt(index, value) needs Function ABI with two UINT64s. initialize is
# unused (zero slots). Missing RPC → skip.
set -euo pipefail
...
if [[ -z "$python" ]]; then
  echo "xrpl-alphanet-vec: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-alphanet-vec: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-alphanet-vec: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$nid" != "21337" || "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-alphanet-vec: skip: not AlphaNet SmartContract ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-alphanet-vec: $RPC $info" >&2

echo "xrpl-alphanet-vec: building XrplVec.wasm" >&2
lake exe pf -- build --target xrpl-alphanet --out "$root/build/xrpl-alphanet" XrplVec
wasm="$root/build/xrpl-alphanet/XrplVec.wasm"
...
# setAt(1, 5) → xs_1 = 5. Do not call initialize (unused seed param).
printf '{"rpc_url":"%s","wallet_seed":"%s","contract_account":"%s","function_name":"setAt","parameters":["1","5"]}\n' \
...
printf '{"rpc_url":"%s","wallet_seed":"%s","contract_account":"%s","function_name":"get1"}\n' \
  "$RPC" "$WALLET" "$contract" >"$cfg"
get_out="$(node "$here/alphanet-rpc.js" call "$cfg")"
echo "$get_out" >&2
"$python" -I -S -c 'import json,sys; d=json.load(sys.stdin); assert d.get("result")=="tesSUCCESS" and d.get("vmReturnCode")==5, d' <<<"$get_out"

printf '{"rpc_url":"%s","owner":"%s","contract_account":"%s","key":"xs_1"}\n' \
  "$RPC" "$OWNER" "$contract" >"$cfg"
value="$(node "$here/alphanet-rpc.js" slot "$cfg")"
rm -f "$cfg"
[[ "$value" == "5" ]] || { echo "FAIL: expected xs_1=5, got $value" >&2; exit 1; }

echo "xrpl-alphanet-vec: ok contract=$contract xs_1=5"
