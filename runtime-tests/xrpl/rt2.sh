#!/usr/bin/env bash
# Engineering local-node gate for XrplRt2 parent hash + base fee.
# Missing docker/bedrock → skip (exit 0). Deploys ProofForge XrplRt2.wasm only.
# stamp writes hashLo and fee; both must be > 0. Hash is not pinned.
set -euo pipefail
...
xrpl_init xrpl-local-rt2

wasm="$root/build/xrpl/XrplRt2.wasm"
fixture="$here/fixture"
staged="$fixture/contract/target/wasm32-unknown-unknown/release/Counter.wasm"

if [[ ! -f "$wasm" ]]; then
  echo "xrpl-local-rt2: building XrplRt2.wasm" >&2
...
echo "xrpl-local-rt2: starting local node" >&2
(cd "$fixture" && "$bedrock" node stop >/dev/null 2>&1) || true
...
hash_lo="$(xrpl_slot_u64 "$owner" "$contract" "hashLo")"
fee="$(xrpl_slot_u64 "$owner" "$contract" "fee")"
[[ "$hash_lo" =~ ^[0-9]+$ ]] || { echo "FAIL: hashLo is not a number: $hash_lo" >&2; exit 1; }
[[ "$fee" =~ ^[0-9]+$ ]] || { echo "FAIL: fee is not a number: $fee" >&2; exit 1; }
[[ "$hash_lo" != "0" ]] || { echo "FAIL: parent hash lo was 0" >&2; exit 1; }
[[ "$fee" -gt 0 ]] || { echo "FAIL: base fee was $fee" >&2; exit 1; }

echo "xrpl-local-rt2: ok (hashLo=$hash_lo fee=$fee; engineering only)"
