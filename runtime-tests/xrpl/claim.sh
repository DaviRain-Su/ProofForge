#!/usr/bin/env bash
# Local 2.6.1: lockIn writes due=ledgerSqn+2; only wallet B claims after due.
...
if [[ -z "$python" ]]; then
  echo "xrpl-claim: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-claim: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-claim: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-claim: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-claim: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-claim: $RPC $info" >&2
...
echo "xrpl-claim: restarted $info" >&2

echo "xrpl-claim: building XrplClaim.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplClaim
wasm="$root/build/xrpl/XrplClaim.wasm"
...
# Same ledger as vest.sh: due = sqn+2, so the next Call is still early.
want "$(call_fn claimB "$WALLET_B")" 1 too-early
[[ "$(slot "$OWNER_B" bal)" == "0" || "$(slot "$OWNER_B" bal)" == "missing" ]] || {
  echo "FAIL: early B claim must keep B=0" >&2; exit 1
}

want "$(call_fn claimB)" 3 a-claim
[[ "$(slot "$contract" esc)" == "5" ]] || { echo "FAIL: A claim must keep esc=5" >&2; exit 1; }

want "$(call_fn claimB "$WALLET_B")" 0 claimB
[[ "$(slot "$OWNER_B" bal)" == "5" ]] || { echo "FAIL: after claim B want 5" >&2; exit 1; }
[[ "$(slot "$contract" esc)" == "0" ]] || { echo "FAIL: after claim esc want 0" >&2; exit 1; }
[[ "$(slot "$OWNER" bal)" == "5" ]] || { echo "FAIL: claim must keep A=5" >&2; exit 1; }

rm -f "$cfg"
echo "xrpl-claim: ok contract=$contract lockIn then B claimB after due (not Sdk.Payments)"
