...
if [[ -z "$python" ]]; then
  echo "xrpl-frost: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-frost: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-frost: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-frost: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-frost: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-frost: $RPC $info" >&2
...
echo "xrpl-frost: restarted $info" >&2

echo "xrpl-frost: building XrplFrost.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplFrost
wasm="$root/build/xrpl/XrplFrost.wasm"
...
rm -f "$cfg"
echo "xrpl-frost: ok contract=$contract freeze latch=5 then unlatch (not Sdk.Payments)"
