#!/usr/bin/env bash
# Local 2.6.1: allowance takeB, minter clawB, then A cashSelf.
# Frozen take/cash return 5. Funded first-install Create.
...
if [[ -z "$python" ]]; then
  echo "xrpl-raid: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-raid: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-raid: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-raid: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-raid: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-raid: $RPC $info" >&2
...
echo "xrpl-raid: restarted $info" >&2

echo "xrpl-raid: building XrplRaid.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplRaid
wasm="$root/build/xrpl/XrplRaid.wasm"
...
rm -f "$cfg"
echo "xrpl-raid: ok contract=$contract takeB/clawB then A cashSelf +192-fee (not Sdk.Payments)"
