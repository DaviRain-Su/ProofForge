#!/usr/bin/env bash
# Local 2.6.1: shared supp on the contract AccountID card via Card.storeSelf + Pay.emitToCaller.
...
if [[ -z "$python" ]]; then
  echo "xrpl-cash: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-cash: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-cash: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-cash: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-cash: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-cash: $RPC $info" >&2
...
echo "xrpl-cash: restarted $info" >&2

echo "xrpl-cash: building XrplCash.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplCash
wasm="$root/build/xrpl/XrplCash.wasm"
...
echo "xrpl-cash: ok contract=$contract credit then cash (bal=0 supp=0 emit 192; not Sdk.Payments)"
