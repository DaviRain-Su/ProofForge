#!/usr/bin/env bash
# Local 2.6.1: A cancel before due; after due only wallet B cashB +192.
# Else-if guards. Funded first-install Create. Public still -22.
...
if [[ -z "$python" ]]; then
  echo "xrpl-escape: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-escape: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-escape: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-escape: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-escape: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-escape: $RPC $info" >&2
...
echo "xrpl-escape: restarted $info" >&2

echo "xrpl-escape: building XrplEscape.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplEscape
wasm="$root/build/xrpl/XrplEscape.wasm"
...
echo "xrpl-escape: ok contract=$contract A cancel then B cashB +192-fee (not Sdk.Payments)"
