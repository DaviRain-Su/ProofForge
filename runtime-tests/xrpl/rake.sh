#!/usr/bin/env bash
# Local 2.6.1: allowance takeB then B cashSelf. Frozen take/cash return 5.
...
if [[ -z "$python" ]]; then
  echo "xrpl-rake: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-rake: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-rake: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-rake: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-rake: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-rake: $RPC $info" >&2
...
echo "xrpl-rake: restarted $info" >&2

echo "xrpl-rake: building XrplRake.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplRake
wasm="$root/build/xrpl/XrplRake.wasm"
...
rm -f "$cfg"
echo "xrpl-rake: ok contract=$contract takeB then B cashSelf +192-fee (not Sdk.Payments)"
