#!/usr/bin/env bash
# Local 2.6.1: lockIn writes due=ledgerSqn+2; first refund fails; next ledger refunds.
...
if [[ -z "$python" ]]; then
  echo "xrpl-vest: skip: python3 not found" >&2
  exit 0
fi
if ! command -v node >/dev/null 2>&1; then
  echo "xrpl-vest: skip: node not found" >&2
  exit 0
fi
...
if ! info="$(node "$here/alphanet-rpc.js" info "$cfg")"; then
  echo "xrpl-vest: skip: cannot reach $RPC" >&2
  rm -f "$cfg"
  exit 0
fi
...
if [[ "$smart" != "True" && "$smart" != "true" ]]; then
  echo "xrpl-vest: skip: SmartContract not enabled ($info)" >&2
  rm -f "$cfg"
  exit 0
fi
if [[ "$nid" == "21337" ]]; then
  echo "xrpl-vest: skip: public AlphaNet still -22 / -196" >&2
  rm -f "$cfg"
  exit 0
fi
echo "xrpl-vest: $RPC $info" >&2
...
echo "xrpl-vest: restarted $info" >&2

echo "xrpl-vest: building XrplVest.wasm (Bedrock/get_* names)" >&2
lake exe pf -- build --target xrpl --out "$root/build/xrpl" XrplVest
wasm="$root/build/xrpl/XrplVest.wasm"
...
due="$(slot "$contract" due)"
[[ "$due" != "missing" && "$due" != "0" ]] || { echo "FAIL: after lockIn due want nonzero got $due" >&2; exit 1; }

want "$(call_fn refund)" 1 too-early
[[ "$(slot "$OWNER" bal)" == "5" ]] || { echo "FAIL: early refund must keep A=5" >&2; exit 1; }
[[ "$(slot "$contract" esc)" == "5" ]] || { echo "FAIL: early refund must keep esc=5" >&2; exit 1; }

want "$(call_fn refund)" 0 refund
[[ "$(slot "$OWNER" bal)" == "10" ]] || { echo "FAIL: after refund A want 10" >&2; exit 1; }
[[ "$(slot "$contract" esc)" == "0" ]] || { echo "FAIL: after refund esc want 0" >&2; exit 1; }

rm -f "$cfg"
echo "xrpl-vest: ok contract=$contract lockIn due=$due then refund after next ledger (not Sdk.Payments)"
