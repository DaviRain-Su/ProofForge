...
/-!
The contract-facing EVM SDK. These definitions give source programs stable names and typed static
storage handles while erasing through `@[pf_inline]` to the target-owned component bridge. They do
not add runtime layout objects, EVM operations, IR nodes, or emitter cases.
-/
