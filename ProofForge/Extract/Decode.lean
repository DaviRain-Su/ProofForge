...
private partial def asValNamed (env : Environment) (fuel : Nat) (n : Name) (e : Expr) :
  ...
  if (isConstNamed e ``Eq || isConstNamed e ``BEq.beq || isConstNamed e ``Ne ||
    ...
    | _, _, _ => none
  else if (endsWith e ".xrplFlushBal" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplFlushBal) &&
      e.getAppArgs.size ≥ 1 then
    match asVal env fuel e.getAppArgs[e.getAppArgs.size - 1]! with
    | some v => some (.xrplFlushBal v)
    | none => none
  else if (endsWith e ".xrplPeekOwner" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplPeekOwner) &&
      e.getAppArgs.size ≥ 3 then
    let args := e.getAppArgs
    match asVal env fuel args[args.size - 3]!,
        asVal env fuel args[args.size - 2]!,
        asVal env fuel args[args.size - 1]! with
    | some w0, some w1, some w2 => some (.xrplPeekOwner w0 w1 w2)
    | _, _, _ => none
  else if (endsWith e ".xrplFlushHalt" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplFlushHalt) &&
      e.getAppArgs.size ≥ 1 then
    match asVal env fuel e.getAppArgs[e.getAppArgs.size - 1]! with
    | some v => some (.xrplFlushHalt v)
    | none => none
  else if (endsWith e ".xrplPeekHalt" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplPeekHalt) &&
      e.getAppArgs.size ≥ 3 then
    let args := e.getAppArgs
    match asVal env fuel args[args.size - 3]!,
        asVal env fuel args[args.size - 2]!,
    ...
    | _, _, _ => none
  else if (endsWith e ".xrplFlushCap" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplFlushCap) &&
      e.getAppArgs.size ≥ 1 then
    match asVal env fuel e.getAppArgs[e.getAppArgs.size - 1]! with
    | some v => some (.xrplFlushCap v)
    | none => none
  else if (endsWith e ".xrplPeekCap" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplPeekCap) &&
      e.getAppArgs.size ≥ 3 then
    let args := e.getAppArgs
    match asVal env fuel args[args.size - 3]!,
        asVal env fuel args[args.size - 2]!,
        asVal env fuel args[args.size - 1]! with
    | some w0, some w1, some w2 => some (.xrplPeekCap w0 w1 w2)
    | _, _, _ => none
  else if (endsWith e ".xrplFlushAllw" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplFlushAllw) &&
      e.getAppArgs.size ≥ 1 then
    match asVal env fuel e.getAppArgs[e.getAppArgs.size - 1]! with
    | some v => some (.xrplFlushAllw v)
    | none => none
  else if (endsWith e ".xrplPeekAllw" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplPeekAllw) &&
      e.getAppArgs.size ≥ 3 then
    let args := e.getAppArgs
    match asVal env fuel args[args.size - 3]!,
        asVal env fuel args[args.size - 2]!,
        asVal env fuel args[args.size - 1]! with
    | some w0, some w1, some w2 => some (.xrplPeekAllw w0 w1 w2)
    | _, _, _ => none
  else if (endsWith e ".xrplFlushLock" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplFlushLock) &&
      e.getAppArgs.size ≥ 1 then
    match asVal env fuel e.getAppArgs[e.getAppArgs.size - 1]! with
    | some v => some (.xrplFlushLock v)
    | none => none
  else if (endsWith e ".xrplPeekLock" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplPeekLock) &&
      e.getAppArgs.size ≥ 3 then
    let args := e.getAppArgs
    match asVal env fuel args[args.size - 3]!,
        asVal env fuel args[args.size - 2]!,
    ...
    | _, _, _ => none
  else if (endsWith e ".xrplFlushDue" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplFlushDue) &&
      e.getAppArgs.size ≥ 1 then
    match asVal env fuel e.getAppArgs[e.getAppArgs.size - 1]! with
    | some v => some (.xrplFlushDue v)
    | none => none
  else if (endsWith e ".xrplPeekDue" ||
      isConstNamed e ``ProofForge.Wasm.Xrpl.Runtime.xrplPeekDue) &&
      e.getAppArgs.size ≥ 3 then
    let args := e.getAppArgs
    match asVal env fuel args[args.size - 3]!,
        asVal env fuel args[args.size - 2]!,
        asVal env fuel args[args.size - 1]! with
    | some w0, some w1, some w2 => some (.xrplPeekDue w0 w1 w2)
    | _, _, _ => none
