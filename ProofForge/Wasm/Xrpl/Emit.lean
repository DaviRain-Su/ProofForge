...
open ProofForge.Wasm.Xrpl.IR (Program Method)
open ProofForge.Wasm.Xrpl.Ops
...
def emit (p : Program) : Except String String := do
  let mut lines : Array String := #[]
  lines := lines.push "// PROOF-FORGE-XRPL-BEDROCK v0"
  lines := lines.push s!"// digest={ProofForge.Wasm.Xrpl.IR.digestHex p}"
  lines := lines.push
