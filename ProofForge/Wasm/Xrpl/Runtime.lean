...
@[irreducible] def xrplFlushDue (v : UInt64) : UInt64 :=
  v

/-- Load `due` from the card owned by `(w0,w1,w2)` (missing → 0). Rewrites
persist Owner. Vesting height on the contract card, not a Map. -/
@[irreducible] def xrplPeekDue (w0 w1 w2 : UInt64) : UInt64 :=
  let _ := w0
  let _ := w1
  let _ := w2
  0
