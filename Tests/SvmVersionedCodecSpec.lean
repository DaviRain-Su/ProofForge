...
#guard (Examples.VersionedLedger.fixed 1).header.classify
    Examples.VersionedLedger.ledgerDiscriminator 1 == Status.ready
#guard (Examples.VersionedLedger.fixed 1).header.classify 7 1 == Status.wrongDiscriminator
#guard (Examples.VersionedLedger.fixed 1).header.classify
    Examples.VersionedLedger.ledgerDiscriminator 9 == Status.unsupportedVersion
#guard (Examples.VersionedLedger.fixed 1).header.classify 0 1 == Status.malformed
#guard (Examples.VersionedLedger.fixed 1).header.classify
    Examples.VersionedLedger.ledgerDiscriminator 0 == Status.malformed

-- Static malformed codecs and transition edges fail their compile-time gates.
#guard !({ (Examples.VersionedLedger.fixed 1).header with
  expectedDiscriminator := 0 }).wellFormed
#guard !({ (Examples.VersionedLedger.fixed 1).header with
  supportedVersion := 0 }).wellFormed
#guard !({ (Examples.VersionedLedger.fixed 1).header with
  versionField := Field.scalar 1 3 }).wellFormed
#guard !(Transition.from (Examples.VersionedMigrator.fixed 1).header 0).wellFormed
#guard !(Transition.from (Examples.VersionedMigrator.fixed 1).header 2).wellFormed
