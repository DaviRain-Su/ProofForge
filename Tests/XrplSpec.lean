...
#guard ProofForge.Wasm.Xrpl.Registry.digestOf "XrplClamp" == some "d10847e4aee069a2"
#guard ProofForge.Wasm.Xrpl.Registry.digestOf "XrplClasp" == some "44ca3ba8ccd77e29"
...
#pf_xrpl_build Examples.XrplFrost
...
#pf_xrpl_build Examples.XrplClamp

#pf_xrpl_build Examples.XrplClasp

#pf_xrpl_build Examples.XrplCleft
...
#pf_xrpl_build Examples.XrplRamp
...
open Lean Elab Command in
elab "#pf_xrpl_frost_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing frost anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-frost: {source.length} bytes of WAT passed frost anchor check"

#pf_xrpl_frost_emit_check Examples.XrplFrost

open Lean Elab Command in
elab "#pf_xrpl_glaze_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing glaze anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-glaze: {source.length} bytes of WAT passed glaze anchor check"

#pf_xrpl_glaze_emit_check Examples.XrplGlaze

open Lean Elab Command in
elab "#pf_xrpl_hinge_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing hinge anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
...
open Lean Elab Command in
elab "#pf_xrpl_clamp_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing clamp anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-clamp: {source.length} bytes of WAT passed clamp anchor check"

#pf_xrpl_clamp_emit_check Examples.XrplClamp

open Lean Elab Command in
elab "#pf_xrpl_clasp_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing clasp anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-clasp: {source.length} bytes of WAT passed clasp anchor check"

#pf_xrpl_clasp_emit_check Examples.XrplClasp

open Lean Elab Command in
elab "#pf_xrpl_cleft_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing cleft anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-cleft: {source.length} bytes of WAT passed cleft anchor check"

#pf_xrpl_cleft_emit_check Examples.XrplCleft

open Lean Elab Command in
elab "#pf_xrpl_clip_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing clip anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-clip: {source.length} bytes of WAT passed clip anchor check"

#pf_xrpl_clip_emit_check Examples.XrplClip

open Lean Elab Command in
elab "#pf_xrpl_clot_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing clot anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
...
open Lean Elab Command in
elab "#pf_xrpl_rate_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        let anchors : Array String := #[
          "(func (export \"credit\") (result i32)",
          "(func (export \"cash384\") (result i32)",
          "(func (export \"freeze\") (result i32)",
          "(func (export \"pause\") (result i32)",
          "(i32.const 524313)",
          "(import \"host_lib\" \"emit_built_txn\"",
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing rate anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-rate: {source.length} bytes of WAT passed rate anchor check"

#pf_xrpl_rate_emit_check Examples.XrplRate

open Lean Elab Command in
elab "#pf_xrpl_crate_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        let anchors : Array String := #[
          "(func (export \"credit\") (result i32)",
          "(func (export \"grant\") (result i32)",
          "(func (export \"takeAndLock\") (result i32)",
          "(func (export \"cancel\") (result i32)",
          "(func (export \"cashB\") (result i32)",
          "(i32.const 524313)",
          "(import \"host_lib\" \"emit_built_txn\"",
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing crate anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
...
open Lean Elab Command in
elab "#pf_xrpl_revoke_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing revoke anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
...
open Lean Elab Command in
elab "#pf_xrpl_rake_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing rake anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
...
open Lean Elab Command in
elab "#pf_xrpl_ramp_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing ramp anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-ramp: {source.length} bytes of WAT passed ramp anchor check"

#pf_xrpl_ramp_emit_check Examples.XrplRamp

open Lean Elab Command in
elab "#pf_xrpl_rank_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing rank anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-rank: {source.length} bytes of WAT passed rank anchor check"

#pf_xrpl_rank_emit_check Examples.XrplRank

open Lean Elab Command in
elab "#pf_xrpl_rant_emit_check " n:ident : command => do
  let env ← getEnv
  match Extract.extractModuleIR env n.getId none >>= ProofForge.Wasm.Xrpl.IR.fromExtracted with
  | .error reason => throwError reason
  | .ok program =>
    ...
    | .ok source => do
        ...
        for anchor in anchors do
          unless source.contains anchor do
            throwError s!"wasm emit is missing rant anchor: {anchor}\n{source}"
        unless !source.contains "Sdk.Payments" do
          throwError "wasm emit must not mention Sdk.Payments"
        unless !source.contains "Sdk.Map" do
          throwError "wasm emit must not mention Sdk.Map"
        logInfo m!"proofforge-xrpl-rant: {source.length} bytes of WAT passed rant anchor check"

#pf_xrpl_rant_emit_check Examples.XrplRant
