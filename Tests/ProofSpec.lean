...
- `Examples.Tree.removeNode_size` / `init_wf` / `allocNode_wf` / `insertRoot_wf` / `paintNode_wf` / `linkLeft_wf` / `linkRight_wf`
...
#check Examples.Tree.rotateLeft_wf
#check Examples.Tree.rotateRight_wf
