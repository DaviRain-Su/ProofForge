...
- SVM registry 50 个程序 / 50 个 Mollusk integration 文件；这表示每个程序有门，不表示每个入口都已有链上矩阵。全量 `pf build` 与 Mollusk 205/205 当前通过。
...
| P5 动态 Phoenix-v1 | 部分：profile + metadata/header + complete bid tree/free-list + 本地部署门已有 | 固定/固定-stride有界 account word、parent walk 与 fixed stack bitmap | canonical profile、sequence、allocator envelope、selected path，以及 bid 全树 ordering/RB invariants/exact live-free partition 已进 Lean/Mollusk；ask/trader whole-tree coverage、节点写入、remaining accounts 和完整 Phoenix-v1 兼容仍 fail closed |
