...
- Solanalib 首个真实 CFG correspondence：Counter checked-add 的 Core operands / physical slot / success-overflow edge 必须一致；typed `jgt`、`r10-24` scratch handoff、static store 由上游 small-step semantics 执行，theorem 分别覆盖任意 64-bit guard、成功写回和 overflow 内存不变

## 当前状态

- `lake build Tests` 当前 187 jobs；68 个 imported test modules 含 761 个 `#guard` / `#guard_msgs`。
...
1. Solanalib 继续补 sub/mul/div/mod checked edge 与普通 CFG branch；checked-add control/instruction correspondence 已完成。
