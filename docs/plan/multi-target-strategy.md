...
- **剩余**：N13 handle Extract 已收口（`And3`..`And8`）；N14 Vector+Queue+Lookup Handle 已落地（Iterable follow-up）；三 target conformance（N15）
...
| **N14** | [`wsm-near-store-meta-001`](tasks/wsm-near-store-meta-001.md) | ~~collection prefix/metadata Handle~~（Vector+Queue+Lookup done；仍 **不** 冒充 near-sdk `Drop`/cache） | N5 + N9 ✓ | ✓ Vector+Queue+Lookup Handle；digests unchanged |
