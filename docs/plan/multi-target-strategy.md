...
| 优先级 | 包 ID 建议 | 内容 | 验收 |
|---|---|---|---|
| P0 | [`evm-rt-nested-001`](tasks/evm-rt-nested-001.md) | nested/constructed/wide dynamic return 与 aggregate storage 组合 | Lean + solc + Anvil malformed matrix（wide/constructed **Anvil 已通**；`nestedRecord` + `EvmAggregateStorage` Lean slice **已通**；Anvil matrix for aggregate storage 仍开） |
