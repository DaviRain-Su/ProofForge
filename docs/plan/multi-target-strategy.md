...
- **剩余**：N13 handle Extract 已收口（`And3`..`And8` + N>8 fail-closed）；N14 Vector+Queue+Lookup+Iterable Handle 已落地；N15 Counter+Token digest 表已钉 + runtime sandbox matrix pointers 已钉（✓ TokenShape 共享 transfer stub；NEAR approve = NEP-141 N/A）
...
| **N15** | [`wsm-near-conformance-001`](tasks/wsm-near-conformance-001.md) | Counter + Token-shaped cross-target digest 表 | N12d | ✓ Counter 三 target；✓ Token approve/xfer digests（target-local）；✓ TokenShape 共享 transfer stub；NEAR approve = NEP-141 N/A；✓ runtime sandbox matrix pointers |
...
| **erg-evm-effect-001** | [`erg-evm-effect-001`](tasks/erg-evm-effect-001.md) — `Effect.ensure` / Token-shaped sequential surface | Evm.Sdk + `Examples/EvmTokenErgonomics.lean` + Token approve/transfer/transferFrom/mint/burn/burnFrom/allowance/pause/unpause/permit | ✓ ergonomics digest `138c08a82e1ad205`; ✓ Token Bool ABI trio → `f6a3fcbc3c7331fe`; ✓ mint/burn/burnFrom/allowance/pause/unpause/permit `Effect.ensureCode` → digest `7d01d10202d87dd3` |
