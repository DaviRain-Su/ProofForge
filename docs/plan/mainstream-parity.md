...
|---|---|---|---|
| AccountInfo | fixed Account/Signer handle；key/owner/data words、lamports/length/flags；read-only bounded remaining-account view；program-owned checked data word writes；fixed-handle checked lamport transfer 在同一预检中验证 writable/source owner/balance/destination overflow/canonical alias，允许 foreign-owned destination | 一等 32-byte address；account resize/zero-fill；完整 initialization/close helpers；AccountView 与 lamport mutation 同时使用时的 alias-aware variable walk | F0/F1 |
