...
|---|---|---|---|
| AccountInfo | fixed Account/Signer handle；一等 compiler-erased `Pubkey` 值：key/owner 投影、全四字 equality/inequality 与 canonical matching（`R3-023`）；key/owner/data words、lamports/length/flags；read-only bounded remaining-account view；program-owned checked data word writes；fixed-handle checked lamport transfer；fixed external handle 的 zero-initializing resize 验证 managed-state alias/writable/current owner/10 MiB ceiling/+10,240 original-growth（`R2-010`） | Pubkey 作为整体 entry 参数/返回值的 aggregate binding；完整 initialization/close/rent-top-up helpers；AccountView 与 direct mutation 同时使用时的 alias-aware variable walk | F0/F1 |
