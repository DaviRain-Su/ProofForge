...
| invocation-local | bounded scratch/heap region，OOM 显式失败，调用结束即失效 | SVM `Sdk.Transient.Vector64` 与 `Bytes` 各有两个可同时 active 的编译期 slot，提供 begin/push/pop/set/truncate/clear/finish/length/get、固定 byte append/log-data、独立 metadata/payload 与真实 OOM | generic POD element/record writer、更多 resource-manifest slots、insert/remove-at/iteration；EVM bounded memory binding |
...
| memory | official-shaped 32 KiB downward bump model、bounded scratch plans；checked account spans 已绑定 `memcpy/memmove/memcmp/memset`；source-visible bounded `Vector64` 与 checked byte writer 已绑定真实 heap，各有两个可同时 active 的同类型 slot，支持 checked LIFO pop 与 shared Rust-style truncate 并保留显式 OOM/full/OOB/state/range errors；共享 lifecycle interpreter 统一 allocation/metadata/clear/truncate/finish | generic POD vectors/record writer、更多 manifest-bounded handles、bounded insert/remove/iteration；allocator/resource manifest | F0/F1 |
...
| assets | `Fungible.Balances` O(1) checked movement；`Fungible.Allowances` explicit pair-handle approve/checked increase/decrease/spend；`Erc721` bounded 192-bit-key owner/per-token approval/operator/balance/mint/transfer/burn core；Token/Credits/Vault/Ownable 与 Collectible/Badge 独立消费，wrap/alias/over-spend/token-id truncation 已由 Anvil 验收 | ERC-721 standard Address views/full-width id/receiver callback；bounded ERC-1155；standard typed events/errors | F2 |
