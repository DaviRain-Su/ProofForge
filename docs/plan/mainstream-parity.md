...
| SPL Token | Runtime 已有 classic transfer/mint/burn/approve/revoke/authority/freeze/account/multisig/sync-native recipes | 统一 `Svm.Sdk.Token` facade、state parsers、program-id policy、signed/fixed-account variants | F1 |
...
| access/safety | owner/running/two-step ownership、bounded static roles | Pausable events, ReentrancyGuard, enumerable/admin roles, timelock/access manager | F1/F2 |
| calls/payments | typed closed call result、safe ETH/ERC-20-shaped interactions | reusable Address/SafeERC20 policy、revert bubbling、code check、pull payment/multicall | F1/F2 |
