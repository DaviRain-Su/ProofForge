...
| memory | official-shaped 32 KiB downward bump model、bounded scratch plans；checked account spans 已绑定 `memcpy/memmove/memcmp/memset`；source-visible bounded `Vector64` 与 checked byte writer 已绑定真实 heap，支持 checked LIFO pop 与 shared Rust-style truncate，可异类并存并保留显式 OOM/full/OOB/state/range errors；共享 lifecycle interpreter 统一 allocation/metadata/clear/truncate/finish | 同类型多 handle、generic POD vectors/record writer、bounded insert/remove/iteration；allocator/resource manifest | F0/F1 |
...
| environment | caller/origin/self/coinbase、`msg.sig`、block number/hash、timestamp/chain id/value/selfBalance/immutables；full-width gasleft/gasprice/basefee/prevrandao/gaslimit/blockhash；Address balance/codeSize/codeHash；solc 0.8.34 + Cancun target pin | bounded `msg.data`、broader target-version matrix；raw code bytes 不进入 safe SDK，blockhash input 暂为 checked UInt64 block height | F1 |
| call/create | closed ERC-20/WETH/router/permit CALL/STATICCALL、typed ≤32-byte result policies、safe ETH send、explicit ordered reentrancy guard；`Address.hasCode` 提供诚实的观察点 runtime-code predicate | bounded generic call data/result/revert bubbling；static/delegate semantics；CREATE/CREATE2；把 code observation 与具体 safe-call result policy 组合 | F1/F2 |
...
| calls/payments | `Evm.Sdk.Payments` bounded Ether/ERC20/WETH/fixed-router facade；Vault/TipJar 只消费 SDK，typed closed result policy 保持不变；`Address.hasCode` 暴露不自动注入调用的 runtime-code observation policy | revert bubbling、code observation 与具体 safe-call result 的组合、pull payment/multicall；arbitrary call 继续留在 advanced boundary | F1/F2 |
