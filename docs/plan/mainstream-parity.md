...
| environment | caller/self/coinbase、block number/hash、timestamp/chain id/value/balance/immutables；full-width gasleft/basefee/prevrandao/gaslimit/blockhash；solc 0.8.34 + Cancun target pin | code/codehash and broader target-version matrix；blockhash input 暂为 checked UInt64 block height | F1 |
