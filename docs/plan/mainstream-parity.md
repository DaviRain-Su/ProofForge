...
| memory | official-shaped 32 KiB downward bump model、bounded scratch plans；checked account spans 已绑定 `memcpy/memmove/memcmp/memset`；source-visible bounded `Vector64`（含 checked LIFO pop）与 checked byte writer 已绑定真实 heap，可异类并存并保留显式 OOM/full/OOB/state/range errors；共享 lifecycle interpreter 统一 allocation/metadata/clear/finish | 同类型多 handle、generic POD vectors/record writer；allocator/resource manifest | F0/F1 |
...
| environment | caller/origin/self/coinbase、block number/hash、timestamp/chain id/value/selfBalance/immutables；full-width gasleft/gasprice/basefee/prevrandao/gaslimit/blockhash；Address balance/codeSize/codeHash；solc 0.8.34 + Cancun target pin | `msg.sig`、bounded `msg.data`、broader target-version matrix；raw code bytes 不进入 safe SDK，blockhash input 暂为 checked UInt64 block height | F1 |
