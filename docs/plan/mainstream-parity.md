...
| environment | caller/self/coinbase、block number/hash、timestamp/chain id/value/balance/immutables；full-width gasleft/basefee/prevrandao/gaslimit/blockhash；Address codeSize/codeHash；solc 0.8.34 + Cancun target pin | broader target-version matrix；raw code bytes 不进入 safe SDK，blockhash input 暂为 checked UInt64 block height | F1 |
