...
| 生命周期 | 正确表示 | 当前状态 | 必须补齐 |
|---|---|---|---|
| source/ABI 边界 | compile-time capacity + runtime length + fixed scalar frame | shared `BoundedVec` operations、Map/Set/Queue/BitSet laws、distinct bounded bytes/string + strict UTF-8 source contract + active-prefix equality/unsigned lexicographic ordering/exact substring/prefix/suffix/typed first-position search；SVM Borsh 与 EVM ABI generic bounded/bytes/string input 已有；两边 top-level one-limb bounded/tagged return 已有独立 target plan；packed BitSet 与 enumerable Set 的纯策略已由 SVM/EVM 分别绑定持久状态 | richer collection persistence bindings、nested/constructed tagged shapes、wide/aggregate dynamic elements |
| invocation-local | bounded scratch/heap region，OOM 显式失败，调用结束即失效 | SVM `Sdk.Transient.Vector64` 与 `Bytes` 各有两个可同时 active 的编译期 slot；`ResourceManifest` 声明 slot 预算（默认 2，`>2` 当前 fail-closed）；`Record64` 在同一 Vector64 slot 上提供派生 geometry、aligned count/access 和 fixed-arity `append1..4` whole-record preflight；typed `Vector128`/`Vector256`/`VectorPubkey` 再提供 whole-value access/mutation；共同提供 begin/pop/truncate/clear/finish、独立 metadata/payload 与真实 OOM | live third+ slots（需 scratch relayout）、insert/remove-at/iteration；EVM bounded memory binding |
...
|---|---|---|---|
| integers/fixed bytes | Bool、u8/16/32/64、allocation-free u128/u256/`FixedBytes n`；shared checked u128/u256→u8/u16/u32/u64 narrowing；shared allocation-free UInt64 min/max/floor-average/checked-ceilDiv、explicit saturating add/sub/mul、floor/ceiling log2/log10/log256、integer sqrt、exact-intermediate floor/ceiling mulDiv 与 explicit-scale floor/ceiling fixed-point mul/div；u256 EVM arithmetic 已覆盖主要 unsigned op | signed widths、u128/u256 root/saturation、typed fixed-point values/casts、统一 overflow vocabulary | F0/F1 |
...
| control/resources | checked arithmetic、bounded `for`、fixed scalar frame、generic fixed-scalar `Except` do-bind | per-method collection/codec/memory budget manifest；禁止隐式 allocation/clone/format | F1 |
...
| 组件 | 当前 | 主要缺口 | 优先级 |
|---|---|---|---|
| System/PDA | static ASCII PDA；non-seeded transfer/create/assign/allocate/nonce；generic compile-time ASCII seeded allocate/create/assign/transfer with checked bincode length；regular/seeded/PDA create 可把 compile-time space 与当前 Rent minimum 组合；fixed program-account close/refund 由 `Account.Handle.closeTo` 组合；resize rent top-up 由 `topUpRentExempt` / `resizeDataWithRentTopUp` 显式组合（`svm-sdk-001`）；program-owned owner-reassign **n/a**（`svm-sdk-002`）；System.assign 仅 inbound system→current program | runtime-selected geometry | F1 |
...
| 类别 | 当前 source surface | 主要缺口 | 优先级 |
|---|---|---|---|
| values/ABI | typed scalars、Address/u128/u256/bytesN、shared checked u128/u256→u8/u16/u32/u64 narrowing、effect-prefixed static wide result frames、static aggregates、tagged input、bounded dynamic-array/bytes/string input、top-level one-limb bounded/tagged output、strict UTF-8 | signed ints、nested/constructed wide dynamic return、dynamic constructor/fallback returndata | F0/F1 |
...
| log/error | typed bounded LOG0..4/custom-error plans behind closed events/errors；source enum 的 zero-argument named errors，以及 1..4 个显式唯一 UInt64 字段的 parameterized errors（Core typed frame → EVM selector/ABI/revert） | wider/structured/dynamic error fields；source-declared parameterized typed events、dynamic indexed/data encoding、Panic/revert-data propagation | F1 |
...
| 组件 | 当前 | 主要缺口 | 优先级 |
|---|---|---|---|
| collections/math | shared checked u128/u256→u8/u16/u32/u64 SafeCast、shared UInt64 min/max/floor-average/checked-ceilDiv/saturating add-sub-mul/floor-and-ceiling log2/log10/log256/integer-sqrt/full-intermediate floor/ceiling-mulDiv/explicit-scale fixed-point mul-div、bounded bytes/String active-prefix equality + unsigned lexicographic ordering + exact substring/prefix/suffix + typed first-position search、typed maps、static declarations、capacity-2 role set、checked u256 operations、persistent bounded UInt64 vector/bitmap/ring/enumerable set/map/checkpoints | signed math、u128/u256 root/saturation、typed fixed-point values/casts、String/Bytes concat/replace/pattern/normalization/collation、Unicode scalar indexes、richer keys/values/elements、wider checkpoints and iteration shapes | F0/F2 |
