...
| packed/Borsh entry and optional return plans | `Svm.EntryAdapter` | entry adapter bridge | checked instruction-data decode and return encoding | bounded invocation bytes and fixed scalar locals | Typed UInt128/UInt256/FixedBytes leaves now derive exact little-endian cursor/return plans; annotation-owned option/enum compatibility plans remain, while general aggregate source-schema derivation is R1 work |
