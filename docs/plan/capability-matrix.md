...
| bounded bytes / UTF-8 string logical semantics | `Core.Value` + `Core.Collections` + `Core.Codec` | fixed UInt8 frame, checked active length, reused BoundedVec laws, strict Unicode-scalar UTF-8 validation; no target lowering in this slice | no shared wire, account, storage, heap object, or pointer | Literal capacities and resource accounting reach Extract with distinct bytes/string schemas. Dynamic capacities and zero/over-budget descriptors fail closed; SVM Borsh and EVM ABI bindings remain explicitly rejected until separately qualified |
