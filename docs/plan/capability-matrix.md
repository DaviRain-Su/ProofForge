...
| typed bool/uint/address/fixed-bytes metadata; bounded unit/tuple/record/enum/option/array codec shape; allocation-free `UInt128`/`UInt256`/`FixedBytes n` values; compiler-erased `BoundedVec α capacity` input carrier | `Core.Codec` + `Core.Value` + Extract | target adapter selects Borsh or ABI; descriptor validation owns depth/node/leaf/capacity budgets | fixed logical limbs or fixed extracted scalar frame; no shared wire/account/storage layout | Literal Lean tuple/record/enum/option/Vector schemas and scalar target bindings are available; SVM/EVM independently bind static aggregates, tagged inputs, and bounded variable inputs. `BoundedVec`'s host Vector does not survive extraction. Recursive/polymorphic/malformed/over-budget shapes fail closed |
...
| PDA seeds, System/Token/ATA/Memo wrappers, generic CPI words | `Svm.Runtime` + `Svm.Sdk.Pda` + `.System` + `.Token` + `.AssociatedToken` + `.Memo` + `Svm.Ops.invoke` + `Svm.Scratch` | target invoke effect | typed bounded instruction plan + `sol_invoke_signed_c` | fixed 1,024-byte CPI stack bank; compile-time regions and invocation-only lifetime | Static ASCII PDA, non-seeded and generic compile-time ASCII seeded System, classic Token, role-typed static ATA Create/CreateIdempotent/RecoverNested, and bounded compile-time ASCII Memo have compiler-erased SDK names (`R3-004..010`); Token/ATA descriptors use CPI-relative handles and application-owned concrete layouts. Seeded System validates 1–32 ASCII bytes and its adjacent bincode length only for the four closed System geometries. Memo validates ≤512 seven-bit bytes only for exact Memo geometry; other generic CPI words remain independent. ATA receives caller-selected Token and ATA programs rather than defaulting classic Token. Dynamic signed self-CPI composes existing scratch plans. Runtime-selected Memo/account geometry remains unavailable; canonical Token/ATA program-id policy is not yet authenticated; unknown TLV semantics remain fail closed |
...
| unified `Svm.Sdk` facade | `Svm.Sdk.Account` + `.Pda` + `.System` + `.Token` + `.AssociatedToken` + `.Memo` + `.Storage` + `.Queue` + `.Transient` | composes existing Runtime leaves, AccountView, `AccountStorage.Source`, Heap/Scratch, and target components | fixed/bounded read gates, static/fixed-account CPI facades, checked storage/tree/allocator routines, and transient reservation plans; no new recipe opcode | compile-time physical/CPI-relative account handles, windows/seeds/payloads, fixed account words with one-based indexes/`0` sentinels, or invocation-local bounded memory | Fixed Account/Signer/view, static PDA, non-seeded/generic ASCII-seeded System, classic Token, static role-typed ATA, bounded static Memo, persistent Field/Vec/Queue/Map/RBMap/allocator, and transient buffer/vector/writer/codec plans have independent consumers. Runtime-selected Memo bytes/account geometry, Token parsers/program-id policy, rent-aware resize, and Token-2022 extension semantics remain R3 work |
...
| static scalar/record/fixed-array declarations plus typed address/address-pair maps | `Evm.Sdk.Storage.Static` + `Evm.Sdk.Storage` + `Evm.HashedMap` | ordinary State extraction for static fields; `Component.hashedMap` for maps | direct `sload`/`sstore` for static slots; typed hash namespace for maps | two disjoint compile-time cursors; descriptors never enter runtime storage | Available (`R5-002`): Bool/u8/u16/u32/u64, Address, UInt256/Bytes32, flat records, fixed arrays and record arrays validate exact consecutive slots. Invalid widths/names/duplicate flattened names fail closed. Static declarations do not move hashed-map bases; generic handle-driven runtime load/store remains future Extract work |
| Ether, Event, Revert, receive/payable | `Evm.Sdk.Payments` + `Evm.Sdk` + `Evm.NativeFx` | `Component.nativeFx` | CALL value, LOG, revert, receive | EVM call frame/log/storage effects | Closed Ether accept/send/receive is consumed through the SDK by Vault and TipJar (`R5-005`); arbitrary event/error recipes, hidden state writes, and inferred reentrancy protection fail closed |
...
| reusable contract policy (access, roles, pause, reentrancy, token/NFT ledgers) | `Evm.Sdk.Access` + `Evm.Sdk.Roles` + `Evm.Sdk.Pausable` + `Evm.Sdk.Fungible` and later components | composition of static state/hashed-map/native/closed-call components | explicit source calls to reads/writes/logs/reverts | fixed Address/scalar fields, typed static declarations, and compile-time hashed namespaces | Access foundation provides owner gates and fixed single-pending two-step ownership. `Roles.Set2` provides capacity-2 membership/grant/revoke decisions (`R5-003`); `Pausable` owns canonical fail-closed u8 flags/transitions (`R5-004`). `Fungible.Balances` owns O(1) checked balance movement (`R5-006/007`); `Fungible.Allowances` owns explicit pair-handle approve/checked increase/decrease/spend (`R5-008`). Token/Credits/Vault/Ownable independently reuse slices. Authorization, cap, permit ownership, events, and typed State writes stay application-owned. Dynamic roles/indexed Address returns, typed pause events, reentrancy, ERC-721, and bounded ERC-1155 remain fail closed |
