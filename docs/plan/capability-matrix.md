...
| typed bool/uint/address/fixed-bytes metadata; bounded unit/tuple/record/enum/option/array codec shape | `Core.Codec` + Extract | target adapter selects Borsh or ABI; descriptor validation owns depth/node/leaf/capacity budgets | no shared physical layout | Scalar metadata and bounded descriptor are available; shared `FixedBytes n`/u128 source values and aggregate target lowering remain R1 work; malformed/over-budget shapes fail closed |
...
| packed/Borsh entry and optional return plans | `Svm.EntryAdapter` | entry adapter bridge | checked instruction-data decode and return encoding | bounded invocation bytes | Typed codec metadata now survives into SVM IR, but decoding still uses registered static plans; schema-driven exact-cursor Borsh lowering remains R1/R3 |
...
| Source/API surface | Owner | Component bridge | Target effect | Physical state/resource | Current status / fail-closed edge |
|---|---|---|---|---|---|
| typed ABI scalar metadata, `Address`, `UInt256`, `Bytes32`, Context, Immutable | `Core.Codec` + `Evm.Codec` + `Evm.Sdk` | `wideWord` where multi-limb logic is required | typed selector/calldata guard/ABI plus environment and checked packed operations | Yul stack/memory; no persistent SDK object | Selector, scalar guards, wide carriers, and ABI JSON consume typed metadata; aggregate ABI shapes and complete bitwise/div/mod remain R1/R4 |
