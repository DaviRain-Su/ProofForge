...
| bounded remaining-account view, runtime-safe index, read-only header/data gate | `Svm.AccountView` + `.Source` | `Component.accountView` | index validated against compile-time capacity and runtime `NUM_ACCOUNTS`, bounded runtime account-count walk, checked header/data read | compile-time `base`/`capacity` window over fixed account headers; read-only, no pointers, no scratch | Available (`R2-001`); window OOB, available-count OOB, duplicate-key `NON_DUP_MARKER`, and short data fail atomically with `Custom(1)`; writes and runtime geometry fail closed |
| PDA seeds, System/Token wrappers, generic CPI words | `Svm.Runtime` + `Svm.Ops.invoke` + `Svm.Scratch` | target invoke effect | typed bounded instruction plan + `sol_invoke_signed_c` | fixed 1,024-byte CPI stack bank; compile-time regions and invocation-only lifetime | Static invoke and dynamic signed self-CPI consume the same typed metas/descriptor/data/infos plan (`R2-002`); malformed bank/alignment/duplicate region/OOM fail before emission. Broader return-data/multi-seed composition and unknown TLV semantics remain fail closed |
...
| unified `Svm.Sdk` facade | `Svm.Sdk.Account` + `.Storage` + `.Queue` | composes existing Runtime leaves, AccountView, `AccountStorage.Source`, and target components | fixed/bounded read gates plus checked storage/tree/allocator routines; no new recipe opcode | compile-time account indexes/windows and fixed account words with one-based indexes/`0` sentinels | Fixed Account/Signer and bounded remaining-account handles are reused by Trio/AccountView; persistent POD Field, bounded Vec/Queue, ordered Map/RBMap, allocator, and canonical initialization are reused by JobQueue/TicketLine. PDA/System/Token and transient Scratch facades remain R3 work |
