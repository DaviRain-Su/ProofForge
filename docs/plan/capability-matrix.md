...
| PDA seeds, System/Token wrappers, generic CPI words | `Svm.Runtime` + `Svm.Sdk.Pda` + `Svm.Sdk.System` + `Svm.Sdk.Token` + `Svm.Ops.invoke` + `Svm.Scratch` | target invoke effect | typed bounded instruction plan + `sol_invoke_signed_c` | fixed 1,024-byte CPI stack bank; compile-time regions and invocation-only lifetime | Static ASCII PDA, non-seeded System, and classic Token have compiler-erased SDK names (`R3-004/005/006`); role-typed checked/unchecked transfer descriptors use CPI-relative handles and application-owned concrete layouts. Dynamic signed self-CPI composes existing scratch plans. Malformed ASCII/bank/alignment/duplicate region/OOM fail before emission; seeded System, Token state/program-id policy, and unknown TLV semantics remain fail closed |
...
| unified `Svm.Sdk` facade | `Svm.Sdk.Account` + `.Pda` + `.System` + `.Token` + `.Storage` + `.Queue` + `.Transient` | composes existing Runtime leaves, AccountView, `AccountStorage.Source`, Heap/Scratch, and target components | fixed/bounded read gates, static/fixed-account CPI facades, checked storage/tree/allocator routines, and transient reservation plans; no new recipe opcode | compile-time physical/CPI-relative account handles, windows/seeds, fixed account words with one-based indexes/`0` sentinels, or invocation-local bounded memory | Fixed Account/Signer/view, static PDA/non-seeded System, classic Token, persistent Field/Vec/Queue/Map/RBMap/allocator, and transient buffer/vector/writer/codec plans have independent consumers. Seeded System, ATA/Memo, Token parsers/program-id policy, and Token-2022 extension semantics remain R3 work |
