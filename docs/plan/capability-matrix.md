...
| typed bool/uint/address/fixed-bytes metadata; bounded unit/tuple/record/enum/option/array codec shape; allocation-free `UInt128`/`UInt256`/`FixedBytes n` values | `Core.Codec` + `Core.Value` + Extract | target adapter selects Borsh or ABI; descriptor validation owns depth/node/leaf/capacity budgets | fixed logical limbs only; no shared wire/account/storage layout | Literal Lean tuple/record/enum/option/Vector schemas and scalar target bindings are available; Core exposes source-order static leaf paths plus target-neutral projection/traversal, while SVM and EVM independently bind static aggregates; tagged/dynamic policy and recursive/polymorphic/malformed/over-budget shapes fail closed |
