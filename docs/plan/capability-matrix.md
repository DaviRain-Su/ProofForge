...
| typed bool/uint/address/fixed-bytes metadata; bounded unit/tuple/record/enum/option/array codec shape; allocation-free `UInt128`/`UInt256`/`FixedBytes n` values | `Core.Codec` + `Core.Value` + Extract | target adapter selects Borsh or ABI; descriptor validation owns depth/node/leaf/capacity budgets | fixed logical limbs only; no shared wire/account/storage layout | Literal Lean tuple/record/enum/option/Vector schemas and scalar target bindings are available; Core exposes source-order static leaf paths plus target-neutral projection/traversal, SVM/EVM independently bind static aggregates, and SVM binds canonical tagged Borsh input; EVM tagged policy, bounded dynamic policy, and recursive/polymorphic/malformed/over-budget shapes fail closed |
...
| packed/Borsh entry and optional return plans | `Svm.EntryAdapter` | entry adapter bridge | checked instruction-data decode and return encoding | bounded invocation bytes and fixed scalar locals | Typed scalars plus static record/product/fixed-vector raw parameters derive exact little-endian cursor/local plans and canonical Bool guards; ordinary Lean Option and bounded UInt64-payload enums derive canonical u8-tagged input plans with branch-local payloads; tagged returns, bounded arrays, richer payloads, and generated aggregate ABI remain fail closed |
