...
| bounded bytes / UTF-8 string logical semantics | `Core.Value` + `Core.Collections` + `Core.Codec` | fixed UInt8 frame, checked active length, reused BoundedVec laws, strict Unicode-scalar UTF-8 validation | no shared wire, account, storage, heap object, or pointer | Literal capacities and resource accounting reach Extract with distinct bytes/string schemas. SVM Borsh and EVM standard ABI now have independently qualified input bindings and strict UTF-8 gates. Dynamic capacities, zero/over-budget descriptors, bounded returns, and nested dynamics fail closed |
