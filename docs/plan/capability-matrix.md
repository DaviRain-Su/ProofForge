...
| full-width environment observations | `Evm.Sdk.Context` + `Evm.Sdk.Address` + `Evm.Environment` | `Component.environment` | GAS/GASPRICE/BASEFEE/PREVRANDAO/GASLIMIT/ORIGIN/COINBASE/BLOCKHASH/BALANCE/EXTCODESIZE/EXTCODEHASH plus fixed `msg.sig`, with one cached observation per wide/fixed-byte result | EVM transaction/call/block context; scalar stack projections plus one fixed 32-byte address packing scratch word | Available (`R4-006/008..013`, `R5-011`): `UInt256` observations and full `Address` values never truncate to an implicit UInt64; `Context.selector` remains source-order `Bytes4`, not a numeric selector. Origin is exposed but documented as unsuitable in place of caller authorization. BLOCKHASH accepts a checked UInt64 height and preserves the VM's recent-256-block semantics. `Address.codeSize` is UInt64, `Address.codeHash` is source-order `Bytes32`, and `Address.hasCode` honestly means only `EXTCODESIZE != 0` at the observation point; it is not an EOA/authentication or call-success test. No allocation, storage, call, top-level Ops constructor, or main-emitter recipe. Bounded `msg.data`, raw code bytes, and a full UInt256 block-number input remain fail closed |
