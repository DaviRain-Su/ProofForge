...
| PDA seeds, System/Token/ATA/Memo wrappers, generic CPI words | `Svm.Runtime` + `Svm.Sdk.Pda` + `.System` + `.Token` + `.AssociatedToken` + `.Memo` + `Svm.Ops.invoke` + `Svm.Scratch` | target invoke effect | typed bounded instruction plan + `sol_invoke_signed_c` | fixed 1,024-byte CPI stack bank; compile-time regions and invocation-only lifetime | Static ASCII PDA, non-seeded and generic compile-time ASCII seeded System, classic Token, fixed ATA CreateIdempotent, and fixed Memo have compiler-erased SDK names (`R3-004..008`); role-typed Token transfer descriptors use CPI-relative handles and application-owned concrete layouts. Seeded System validates 1–32 ASCII bytes and its adjacent bincode length only for the four closed System geometries. ATA receives the caller-selected Token program rather than defaulting classic Token. Dynamic signed self-CPI composes existing scratch plans. General ATA/Memo data remain unavailable; canonical Token/ATA program-id policy is not yet authenticated; unknown TLV semantics remain fail closed |
...
| unified `Svm.Sdk` facade | `Svm.Sdk.Account` + `.Pda` + `.System` + `.Token` + `.AssociatedToken` + `.Memo` + `.Storage` + `.Queue` + `.Transient` | composes existing Runtime leaves, AccountView, `AccountStorage.Source`, Heap/Scratch, and target components | fixed/bounded read gates, static/fixed-account CPI facades, checked storage/tree/allocator routines, and transient reservation plans; no new recipe opcode | compile-time physical/CPI-relative account handles, windows/seeds, fixed account words with one-based indexes/`0` sentinels, or invocation-local bounded memory | Fixed Account/Signer/view, static PDA, non-seeded/generic ASCII-seeded System, classic Token, fixed ATA/Memo, persistent Field/Vec/Queue/Map/RBMap/allocator, and transient buffer/vector/writer/codec plans have independent consumers. General ATA/Memo, Token parsers/program-id policy, rent-aware resize, and Token-2022 extension semantics remain R3 work |
