...
| Clock, EpochSchedule, compile-time Rent | `Svm.Sysvar` + `Svm.Sdk.Sysvar` | `Component.sysvar` | fixed `sol_get_clock_sysvar` / `sol_get_epoch_schedule_sysvar` / `sol_get_rent_sysvar` query interpreter | one fixed deep-stack sysvar copy; no account effect, allocation, pointer, or persistent state | Available (`R2-007`): production source leaves no longer add top-level Ops/IR/main-Emit cases; historical Golden constructors delegate to the same interpreter. Remaining fields, signed timestamp semantics, sliced generic sysvars, and runtime-selected Rent geometry remain fail closed |
