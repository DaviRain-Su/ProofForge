...
| invocation-local bounded `Vector64` | `Svm.TransientVec` + `Svm.Sdk.Transient.Vector64` | `Component.transientVec` | begin/push/set/clear/finish effects plus length/get queries, backed by shared official-shaped downward bump emission | fixed `capacity × 8` heap payload, scalar invocation metadata, one active handle; no source-visible address | Available (`R3-012`): full/OOB is `0x1202`, inactive or mismatched capacity is `0x1203`, OOM is `0x1201`; finish closes without reclaiming the bump allocation. Multiple active vectors, generic elements, pop/insert/remove/iteration, byte vectors, and persistent pointers remain fail closed |
...
| full-width environment observations | `Evm.Sdk.Context` + `Evm.Sdk.Address` + `Evm.Environment` | `Component.environment` | GAS/BASEFEE/PREVRANDAO/GASLIMIT/COINBASE/BLOCKHASH/EXTCODESIZE/EXTCODEHASH with one cached observation per wide result | EVM call/block context; scalar stack projections plus one fixed 32-byte address packing scratch word | Available (`R4-006/008/009/010`): `UInt256` observations and full `Address` values never truncate to an implicit UInt64; BLOCKHASH accepts a checked UInt64 height and preserves the VM's recent-256-block semantics. `Address.codeSize` is UInt64 and `Address.codeHash` is source-order `Bytes32`, including distinct nonexistent-account zero behavior. No allocation, storage, call, top-level Ops constructor, or main-emitter recipe. Raw code bytes and a full UInt256 block-number input remain fail closed |
...
| reusable contract policy (access, roles, pause, reentrancy, token/NFT ledgers, persistent collections) | `Evm.Sdk.Access` + `.Roles` + `.Pausable` + `.Reentrancy` + `.Fungible` + `.StorageVec` | composition of static/ordered-static/hashed-map/native/closed-call components | explicit source calls to reads/writes/logs/reverts | fixed Address/scalar fields, typed static declarations, and compile-time hashed namespaces | Access, capacity-2 roles and Pausable own pure gates/transitions (`R5-001/003/004`). `Reentrancy` composes explicit UInt64 handles with ordered enter/leave around application-owned calls; two consumers and a hostile callback prove nested rejection/rollback (`R5-009`). Balances/Allowances own checked O(1) ledger movement (`R5-006..008`). `StorageVec` owns the persistent bounded UInt64 vector contract — compile-time capacity, explicit length, checked active-prefix decisions, visible literal field writes, O(1) documented gas shape — consumed independently by EvmVecLog and EvmVecStack (`R5-010`). Dynamic roles/indexed Address returns, typed pause events, ERC-721, bounded ERC-1155, and richer storage-vector elements remain fail closed |
