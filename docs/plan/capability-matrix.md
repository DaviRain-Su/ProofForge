...
| reusable contract policy (access, roles, pause, reentrancy, token/NFT ledgers) | `Evm.Sdk.Access` + `Evm.Sdk.Roles` + `Evm.Sdk.Pausable` + `Evm.Sdk.Fungible` and later components | composition of static/ordered-static/hashed-map/native/closed-call components | explicit source calls to reads/writes/logs/reverts | fixed Address/scalar fields, typed static declarations, and compile-time hashed namespaces | Access foundation provides owner gates and fixed single-pending two-step ownership. `Roles.Set2` provides capacity-2 membership/grant/revoke decisions (`R5-003`); `Pausable` owns canonical fail-closed u8 flags/transitions (`R5-004`). `Fungible.Balances` owns O(1) checked balance movement (`R5-006/007`); `Fungible.Allowances` owns checked allowance policy (`R5-008`). Ordered lock writes are sound at Runtime level (`R4-005`), but reusable ReentrancyGuard policy and hostile callback validation are not yet published. Dynamic roles/indexed Address returns, typed pause events, ERC-721, and bounded ERC-1155 remain fail closed |
