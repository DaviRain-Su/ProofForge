...
| packed/Borsh entry and optional return plans | `Svm.EntryAdapter` | entry adapter bridge | checked instruction-data decode and return encoding | bounded invocation bytes and fixed scalar locals | Typed UInt128/UInt256/FixedBytes leaves derive exact little-endian cursor/return plans; aggregate source schemas reach SVM IR but aggregate parameters fail closed until the target-owned Borsh binding lands |
