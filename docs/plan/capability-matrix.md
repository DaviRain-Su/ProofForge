...
| typed bool/uint/address/fixed-bytes metadata; bounded unit/tuple/record/enum/option/array codec shape; allocation-free `UInt128`/`UInt256`/`FixedBytes n` values | `Core.Codec` + `Core.Value` + Extract | target adapter selects Borsh or ABI; descriptor validation owns depth/node/leaf/capacity budgets | fixed logical limbs only; no shared wire/account/storage layout | Metadata, bounded descriptor, and literal `1..32` fixed-byte source values are available; SVM exact-cursor Borsh, EVM u128/bytesN binding, and aggregate derivation remain R1 work; dynamic/malformed/over-budget shapes fail closed |
...
| Source/API surface | Owner | Component bridge | Target effect | Physical state/resource | Current status / fail-closed edge |
|---|---|---|---|---|---|
| typed ABI scalar metadata, `Address`, shared `UInt128`/`UInt256`/`FixedBytes n`, Context, Immutable | `Core.Codec` + `Core.Value` + `Evm.Codec` + `Evm.Sdk` | `wideWord` where multi-limb logic is required | typed selector/calldata guard/ABI plus environment and checked packed operations | Yul stack/memory; no persistent SDK object | Existing selector/scalar guards/uint256/bytes32 carriers and ABI JSON consume typed metadata; u128/general bytesN and aggregate ABI plus complete bitwise/div/mod remain R1/R4 |
