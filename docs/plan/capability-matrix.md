...
| reusable contract policy (access, roles, pause, reentrancy, token/NFT ledgers, persistent collections) | `Evm.Sdk.Access` + `.Roles` + `.Pausable` + `.Reentrancy` + `.Fungible` + `.Erc721` + `.Erc1155` + `.StorageVec` + `.StorageBitmap` | composition of static/ordered-static/hashed-map/native/closed-call components | explicit source calls to reads/writes/logs/reverts | fixed Address/scalar fields, typed static declarations, and compile-time hashed namespaces | Access, capacity-2 roles and Pausable own pure gates/transitions (`R5-001/003/004`). `Reentrancy` composes explicit UInt64 handles with ordered enter/leave around application-owned calls; two consumers and a hostile callback prove nested rejection/rollback (`R5-009`). Balances/Allowances own checked O(1) fungible movement (`R5-006..008`). `Erc721` owns bounded owner/approval/operator/balance policy consumed by Collectible/Badge (`R5-013`). `Erc1155` owns bounded single-id balance/operator/movement policy consumed by MultiToken/CraftToken (`R5-014`). Both reject token ids outside a 192-bit key envelope before truncation. `StorageVec` owns the persistent bounded UInt64 vector contract consumed by EvmVecLog/EvmVecStack (`R5-010`). `StorageBitmap` owns packed checked bit policy over static UInt64 words and is consumed by EvmFeatureFlags/EvmClaimBitmap (`R5-015`). Dynamic roles/indexed Address returns, typed standard asset events, receiver callbacks/full-width-id semantics, ERC-1155 batches/metadata, bulk bitmap iteration, and richer storage-vector elements remain fail closed |
