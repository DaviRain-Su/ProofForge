...
| official program-memory spans | `Svm.Memory` + `Svm.Sdk.Memory` | `Component.memory` | one checked `sol_memcpy_` / `sol_memmove_` / `sol_memcmp_` / `sol_memset_` call | compile-time account/offset/length; transient pointer exists only across the syscall | Available (`R2-005`): actual length always checked; writes require writable + current-program owner; memcpy overlap and malformed spans fail closed. Source-visible heap vectors, pointer arithmetic, dynamic spans, and persistent pointers remain unavailable |
