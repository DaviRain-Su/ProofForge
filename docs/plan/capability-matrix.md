...
| reusable contract policy (access, roles, pause, reentrancy, token/NFT ledgers) | `Evm.Sdk.Access` + `Evm.Sdk.Roles` and later components | composition of static state/native/closed-call components | explicit reads/writes/logs/reverts | fixed Address/scalar fields and typed static declarations | Access foundation provides owner/running gates and fixed single-pending two-step ownership. `Roles.Set2` provides capacity-2 membership/grant/revoke slot decisions over two explicit Address fields (`R5-003`); two consumers keep authorization, terminals, and literal writes visible. Dynamic role sets/indexed Address returns, reentrancy, and ERC-721/bounded ERC-1155 remain fail closed |
