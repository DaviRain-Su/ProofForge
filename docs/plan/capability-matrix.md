...
| PDA seeds, System/Token wrappers, generic CPI words | `Svm.Runtime` + `Svm.Sdk.Pda` + `Svm.Sdk.System` + `Svm.Ops.invoke` + `Svm.Scratch` | target invoke effect | typed bounded instruction plan + `sol_invoke_signed_c` | fixed 1,024-byte CPI stack bank; compile-time regions and invocation-only lifetime | Static ASCII PDA bump/check/signed-create and non-seeded System transfer/create/assign/allocate/advanceNonce have compiler-erased SDK names (`R3-004/005`); dynamic signed self-CPI composes instruction and signer-tail plans through `Sdk.Transient.SignedCpiCodec` (`R3-003`). Malformed ASCII/bank/alignment/duplicate region/OOM fail before emission; seeded System/Token facades and unknown TLV semantics remain fail closed |
...
| unified `Svm.Sdk` facade | `Svm.Sdk.Account` + `.Pda` + `.System` + `.Storage` + `.Queue` + `.Transient` | composes existing Runtime leaves, AccountView, `AccountStorage.Source`, Heap/Scratch, and target components | fixed/bounded read gates, static/fixed-account CPI facades, checked storage/tree/allocator routines, and transient reservation plans; no new recipe opcode | compile-time account indexes/windows/seeds, fixed account words with one-based indexes/`0` sentinels, or invocation-local bounded memory | Fixed Account/Signer/view, static PDA/non-seeded System, persistent Field/Vec/Queue/Map/RBMap/allocator, and transient buffer/vector/writer/codec plans have independent consumers. Seeded System and Token facades remain R3 work |
