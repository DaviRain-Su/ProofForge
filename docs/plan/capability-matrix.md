...
| PDA seeds, System/Token/ATA/Memo wrappers, generic CPI words | `Svm.Runtime` + `Svm.Sdk.Pda` + `.System` + `.Token` + `.AssociatedToken` + `.Memo` + `Svm.Ops.invoke` + `Svm.Scratch` | target invoke effect | typed bounded instruction plan + `sol_invoke_signed_c` | fixed 1,024-byte CPI stack bank; compile-time regions and invocation-only lifetime | Static ASCII PDA, non-seeded System, classic Token, fixed ATA CreateIdempotent, and fixed Memo have compiler-erased SDK names (`R3-004..007`); role-typed Token transfer descriptors use CPI-relative handles and application-owned concrete layouts. ATA receives the caller-selected Token program rather than defaulting classic Token. Dynamic signed self-CPI composes existing scratch plans. Seeded System and general ATA/Memo data are unavailable; canonical Token/ATA program-id policy is not yet authenticated; unknown TLV semantics remain fail closed |
...
| ERC-20/WETH/router/permit interaction | `Evm.Sdk` + `Evm.ClosedCall` + `Evm.CallResult` | `Component.closedCall` | typed closed CALL/STATICCALL and one shared result interpreter | closed calldata; ≤32 copied returndata bytes at memory offset zero | Available (`R4-001`): success-only, exact-word, and ERC-20 empty-or-nonzero-word policies fail on call failure/wrong length/zero. Callee/calldata remain closed; arbitrary call, delegatecall, proxy, create, and unbounded returndata fail closed |
| reusable contract policy (access, roles, pause, reentrancy, token/NFT ledgers) | `Evm.Sdk.Access` + `Evm.Sdk.Roles` + `Evm.Sdk.Pausable` and later components | composition of static state/native/closed-call components | explicit reads/writes/logs/reverts | fixed Address/scalar fields and typed static declarations | Access foundation provides owner gates and fixed single-pending two-step ownership. `Roles.Set2` provides capacity-2 membership/grant/revoke slot decisions over two explicit Address fields (`R5-003`). `Pausable` owns canonical fail-closed u8 flags/predicates/replacement transitions (`R5-004`); two consumers keep authorization, events, terminals, and literal writes visible. Dynamic role sets/indexed Address returns, typed pause events, reentrancy, and ERC-721/bounded ERC-1155 remain fail closed |
