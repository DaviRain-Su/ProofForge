...
| PDA seeds, System/Token/ATA/Memo wrappers, generic CPI words | `Svm.Runtime` + `Svm.Sdk.Pda` + `.System` + `.Token` + `.AssociatedToken` + `.Memo` + `Svm.Ops.invoke` + `Svm.Scratch` | target invoke effect | typed bounded instruction plan + `sol_invoke_signed_c` | fixed 1,024-byte CPI stack bank; compile-time regions and invocation-only lifetime | Static ASCII PDA, non-seeded and generic compile-time ASCII seeded System, classic Token, fixed ATA CreateIdempotent, and bounded compile-time ASCII Memo have compiler-erased SDK names (`R3-004..009`); role-typed Token transfer descriptors use CPI-relative handles and application-owned concrete layouts. Seeded System validates 1–32 ASCII bytes and its adjacent bincode length only for the four closed System geometries. Memo validates ≤512 seven-bit bytes only for exact Memo geometry; other generic CPI words remain independent. ATA receives the caller-selected Token program rather than defaulting classic Token. Dynamic signed self-CPI composes existing scratch plans. Runtime-selected Memo bytes and general ATA remain unavailable; canonical Token/ATA program-id policy is not yet authenticated; unknown TLV semantics remain fail closed |
