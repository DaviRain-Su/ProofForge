...
| full-width environment observations | `Evm.Sdk.Context` + `Evm.Environment` | `Component.environment` | GAS/BASEFEE/PREVRANDAO/GASLIMIT/COINBASE/BLOCKHASH with one cached observation per returned value | EVM call/block context; scalar stack projections only | Available (`R4-006/008/009`): `UInt256` observations and full `Address` coinbase never truncate to an implicit UInt64; BLOCKHASH accepts a checked UInt64 height and preserves the VM's 256-block/zero-result semantics. No allocation, storage, call, top-level Ops constructor, or main-emitter recipe. EXTCODESIZE/EXTCODEHASH and a full UInt256 block-number input remain fail closed |
