...
| bounded bytes / UTF-8 string logical semantics | `Core.Value` + `Core.Collections` + `Core.Codec` | fixed UInt8 frame, checked active length, reused BoundedVec laws, strict Unicode-scalar UTF-8 validation | no shared wire, account, storage, heap object, or pointer | Literal capacities and resource accounting reach Extract with distinct bytes/string schemas. SVM Borsh and EVM standard ABI have independently qualified input bindings; SVM also binds top-level bounded output with an independent return plan and output-side UTF-8 gate. Dynamic capacities, zero/over-budget descriptors, EVM bounded output, tagged/nested dynamics, and wide dynamic elements fail closed |
