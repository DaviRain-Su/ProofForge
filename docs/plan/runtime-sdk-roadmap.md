...
> 更新：2026-08-30。本文是 SVM 与 EVM 后续 Runtime / SDK 工作的权威排期；NEAR
> 使用同样的 ownership 原则，但其 host/storage/promise 依赖排期由
> [NEAR Runtime / SDK capability plan](analysis/near-runtime-sdk.md) 单独钉住；
...
| SVM storage SDK | POD field、fixed Vec/Queue、packed BitSet、enumerable UInt64 Set、ordered Map/RBMap、one-based allocator、fixed version/discriminator header、bounded cursor、transient scratch | 账户内 `HashMap`、跨 invocation heap、运行时任意 geometry |
...
| 层 | 已有 | 主要缺口 |
|---|---|---|
| Shared | target registration、typed extension、Core CFG、bounded scalar frame、checked arithmetic/control、allocation-free UInt128/UInt256→UInt8/UInt16/UInt32/UInt64 SafeCast、allocation-free UInt64 min/max/floor-average/checked-ceilDiv/saturating add-sub-mul/floor-and-ceiling log2-log10-log256/integer-sqrt/full-intermediate floor-and-ceiling-mulDiv/explicit-scale floor-and-ceiling fixed-point mul-div、generic fixed-scalar `Except` do-bind、bounded codec schema/resource budget、allocation-free fixed bytes/u128/u256 source values、compiler-erased `BoundedVec` input carrier 与 capacity-preserving operations、bounded Map/Set/Queue/BitSet logical semantics、distinct bounded bytes/UTF-8 string source contracts、aggregate source-schema derivation、target-neutral static projection/rewrite traversal；compiler-owned finite SDK value 可用 representation-free `@[pf_boundary]` 复用 generic static schema/projection/result frame；SVM/EVM scalar、static aggregate、tagged、generic bounded 与 bytes/String input target binding；两边独立的 top-level bounded/tagged output plan；bounded scalar dynamic read；同一 logical schema 的 cross-target plan conformance | signed values/casts、UInt128/UInt256 roots/saturation、typed fixed-point values/casts、collection target bindings、bounded mutation writeback、wide/aggregate dynamic element、nested/constructed tagged output、nested dynamic shapes 与更高 resource ceiling |
...
| SVM Component / SDK | `AccountStorage`、RBMap/allocator/cursor、recorder、FIFO cancellation、program-memory span、checked lamport mutation 与 `AccountData` resize；`Svm.Sdk` 已统一 fixed Account/Signer/bounded view、`Account.Handle.transferLamports` / `.resizeData` / `.closeTo` / `.topUpRentExempt` / `.resizeDataWithRentTopUp`、canonical Pubkey/program id、exact SPL Token base-state views、CPI-relative handles、static ASCII PDA、System、classic Token、role-typed ATA、bounded ASCII Memo、POD Field、fixed Vec/Queue/packed BitSet/enumerable UInt64 Set、ordered Map/RBMap、one-based allocator、fixed version/discriminator header + explicit migration edge、checked account-memory facade，以及可同类双 slot 的 invocation-local buffer/Vector64/writer 和 fixed-width UInt64 `Record64`；Token-2022 mint-close typed facade；program-owned owner-reassign **n/a** fail-closed | runtime-selected ATA/Memo geometry 与更多 Token-2022 extension semantics 尚未统一；UTF-8 Memo + one-word `PayloadTransition` 已由 `svm-sdk-006` 收口；live third+ transient slots remain fail-closed after ResourceManifest gate（svm-sdk-004）until scratch relayout；insert/remove/iteration 仍 fail closed |
...
| EVM SDK | shared UInt128/UInt256→UInt8/UInt16/UInt32/UInt64 SafeCast 与 UInt64 min/max/floor-average/checked-ceilDiv/saturating add-sub-mul/floor-and-ceiling log2-log10-log256/integer-sqrt/full-intermediate floor-and-ceiling-mulDiv/explicit-scale fixed-point mul-div；`Storage.Layout` typed maps、`Storage.Static` declarations/ordered stores、Context/Immutable/Event/Revert；`Payments` bounded Ether/ERC20/WETH/router facade；`Access`/`Roles.Set2`/`Pausable`；`Reentrancy` explicit fail-closed guard；`Fungible.Balances/Allowances` checked ledger policy；`Erc721` bounded owner/approval/operator/balance core；`Erc1155` bounded single-id balance/operator/movement core；persistent bounded UInt64 `StorageVec`/`StorageBitmap`/`StorageRing`/`StorageEnumerableSet`/`StorageEnumerableMap`/`StorageCheckpoints`；honest code observation + safe closed-call result policy | signed math、UInt128/UInt256 roots/saturation、typed fixed-point values/casts 与 richer string/bytes utilities、typed pause/asset events、bounded revert bubbling/generic call policy、ERC-721/1155 receiver callbacks/full-width token ids/standard Address views、ERC-1155 batch/metadata；dynamic indexed Address return；wide-key/value map、wider checkpoints 与 richer persistent element shapes |
...
R5-018 persistent StorageCheckpoints、R5-019 persistent StorageEnumerableMap、R5-020 shared SafeCast、
R5-021 checked UInt32 narrowing/generic scalar `Except` bind、R5-022 checked UInt16 narrowing、
R5-023 checked UInt8 narrowing、R1-024 shared bounded UInt64 math、
...
R1-028 shared UInt64 ceiling logarithms/root、
...
[R2-004](tasks/r2-004.md)、[R2-005](tasks/r2-005.md)、[R2-010](tasks/r2-010.md)、
...
[R5-018](tasks/r5-018.md)、[R5-019](tasks/r5-019.md)、[R5-020](tasks/r5-020.md)、
[R5-021](tasks/r5-021.md)、[R3-026](tasks/r3-026.md)、
...
R1-024 已完成 shared bounded UInt64 math：`Core.Math.UInt64` 以 ordinary inline Lean 提供
min/max、不会构造 input sum 的 floor average，以及 caller-typed zero-denominator checked
ceilDiv。BatchSizer 与 EvmPriceBand 分别拥有 SVM/EVM state、error 和 ABI/account policy；
两个 target SDK facade 只 re-export 同一纯值组件，extraction guard 证明没有 target extension
effect，也没有 Runtime/Ops/IR/Component/Emit、allocation 或 pointer。Signed、saturating、
full-precision 与 wide math 继续 fail closed。详见 [R1-024](tasks/r1-024.md)。

R1-025 已完成 shared saturating UInt64 arithmetic：explicit saturatingAdd/saturatingSub/
saturatingMul 分别通过 max-left、left<right 与 zero + max/left preflight 选择 exact result 或
UInt64 boundary；默认 `+/-/*` checked 语义不变。BatchSizer 与 EvmPriceBand 分别消费同一纯
policy；structural extraction guard 钉住 preflight 与 arithmetic operands，不新增 target
effect、Runtime/Ops/IR/CFG/Component/Emit、allocation 或 pointer。Signed、wide saturation 与
full-precision math 继续 fail closed。详见 [R1-025](tasks/r1-025.md)。

R1-026 已完成 shared UInt64 integer logarithms：floor log2/log10/log256 对零统一返回零，
并分别使用 exact 6/5/7-step bounded local-frame ladder，避免纯 let substitution 把 staged
计算指数复制进 target。BatchSizer/EvmPriceBand 分别绑定 capacity/encoding 与 quote-band
policy；extraction guard 钉住 loop bounds 并拒绝 target extension effect。不新增 Runtime/
Ops/IR/CFG/Component/Emit、allocation、pointer 或 shared layout。
wide/full-precision math 继续 fail closed。详见 [R1-026](tasks/r1-026.md)。

R1-027 已完成 shared UInt64 integer square root：floor `sqrt` 对齐 Rust `u64::isqrt` 和
OpenZeppelin default `Math.sqrt`，使用 exact 5-step magnitude seed、three-halves improvement、
6-step Newton `(estimate, quotient)` scalar frame 与 division correction。structural guard 同时
拒绝把 Newton recurrence 误降成 additive `forAccum`，并拒绝 target extension effect。不新增
Runtime/Ops/IR/CFG/Component/Emit、allocation、pointer 或 shared layout。
wide/full-precision root 继续 fail closed。详见 [R1-027](tasks/r1-027.md)。

R1-028 已完成 shared UInt64 ceiling logarithms/root：`log2Ceil`、`log10Ceil`、
`log256Ceil` 与 `sqrtCeil` 用 guarded predecessor identity 提供 OpenZeppelin upward rounding，
保持 R1-026/027 的 exact bounded ladder/frame，不计算可能溢出的 power/square。SVM/EVM
consumer 分别表达 covering capacity/grid 与 quote band/root policy。不新增 Runtime/Ops/IR/
CFG/Component/Emit、allocation、pointer 或 shared layout。详见 [R1-028](tasks/r1-028.md)。

R1-029 已完成 shared full-precision UInt64 `mulDiv`：四个 safe 32-bit partial products 构造
exact 128-bit product，固定 64-step restoring division 返回 floor quotient；零 denominator 与
`denominator ≤ productHigh` 的 exact quotient-overflow preflight 分别返回 caller-owned typed
errors。SVM/EVM consumer 各自拥有 persistence 和 error policy；不新增 Runtime/Ops/IR/CFG/
Component/Emit、allocation、pointer 或 shared physical layout。ceiling `mulDiv`、signed/
fixed-point 与 wider returned quotient 继续 fail closed。详见 [R1-029](tasks/r1-029.md)。

R1-030 已完成 shared full-precision UInt64 ceiling `mulDivCeil`：floor 与 ceiling API 复用
同一个 private exact-product/restoring-division kernel，ceiling 仅根据最终 remainder 做 checked
increment，并单独拒绝 floor=`UInt64.max` 且 remainder 非零的 rounding overflow。两个 target
consumer 各自保留 typed errors 和 persistence；不新增 Runtime/Ops/IR/CFG/Component/Emit、
allocation、pointer 或 shared physical layout。signed/fixed-point 与 wider returned quotient
继续 fail closed。详见 [R1-030](tasks/r1-030.md)。

R1-031 已完成 shared scaled UInt64 fixed-point policy：`Core.FixedPoint.UInt64` 在 R1-029/030
的 exact floor/ceiling kernel 上提供 `mulDown`/`mulUp`/`divDown`/`divUp`，scale 由 application
显式选择且必须非零；division 另外保留独立 zero-divisor error，所有路径保留 caller-owned
...
R2-007 已把已有 Clock、EpochSchedule 与 compile-time Rent lowering 移到 target-owned
`Svm.Sysvar.Query` 和唯一 `Svm.Sysvar.Emit` interpreter。production source 只经过 generic
Component bridge，historical Golden constructors 也委托同一 interpreter；三个 consumer 的
digest、assembly、IDL 与 ELF 均逐字节不变，Mollusk 14/14。其余字段、signed timestamp、
generic sliced sysvar 与 runtime-selected Rent geometry 仍 fail closed；详见
[R2-007](tasks/r2-007.md)。

R2-008 已在该边界补齐其余 unsigned/Boolean native views：Clock leader-schedule epoch，以及
EpochSchedule leader offset、warmup、first-normal epoch/slot。interpreter 按 official native
`repr(C)` 读取后两项的 offset 24/32，明确不把 33-byte packed/POD layout 当作 host-call 的
40-byte native layout；warmup 只读一个 byte 并规范化为 Bool。signed Clock timestamps、
generic sliced/Instructions sysvar 仍 fail closed；详见 [R2-008](tasks/r2-008.md)。

R2-009 已补齐 fixed-account checked lamport mutation：source 只见两个编译期
`Account.Handle` 与 amount，target-owned `Lamports` Component 在任何 store 前验证两个
writable flag、source current-program ownership、source balance、destination nonoverflow 与
canonical header distinctness。带该 effect 的 static Loader-v3 walk 接受 official backward
duplicate entry（prior index + 七个零字节）并解析到 earlier canonical header；same-canonical
source/destination、forward/self/malformed alias 均 `Custom(1)` 且零写入。foreign-owned writable
destination 与 validated zero no-op 可用；不走 System CPI、不暴露 pointer/runtime index。
AccountView 与该 effect 组合时仍走 variable walk 并拒绝 duplicate，后续再统一两种 walk。
详见 [R2-009](tasks/r2-009.md)。

R2-010 已补齐 official-shaped fixed-account data resize：source 只见编译期 external
`Account.Handle` 和 requested length；target-owned `AccountData` Component 在任何写入前验证
managed-state alias、writable、current-program owner、10 MiB ceiling 和相对 invocation-entry
length 的 10,240-byte growth budget。Component capability 让通用 Loader-v3 walk 把原始长度
保存在固定 stack cells，顺序 shrink/grow 和 backward duplicate alias 不依赖已改变的 current
length。grow 先以 `sol_memset_` 清零新区域再发布长度，所有失败保持 payload/length 原子不变；
不走 System Allocate、不开放 runtime account index、heap realloc、rent top-up 或 close。详见
[R2-010](tasks/r2-010.md)。
...
runtime-selected ATA/Memo geometry 与更多 Token-2022 extension
semantics 仍待完成（UTF-8 Memo + one-word payload migration 已由 `svm-sdk-006` 收口；resize rent top-up 已由 `svm-sdk-001` 收口；owner reassignment 已由 `svm-sdk-002` 标 n/a 永久 fail-closed）。R3-014 又以纯 `pf_inline` `Svm.Sdk.Sysvar` facade 收口已有 Clock、
...
R3-022 再在普通 Lean SDK 层把 fixed-width record 组合到同一 `Vector64`：constructor 只接收
limb stride 与 record capacity，payload/slot geometry 自动派生；`append1..4` 在任何 push 前
...
R3-024 进一步让 compiler-owned finite SDK value 通过 representation-free `@[pf_boundary]`
复用 generic static schema、projection 与 fixed result frame；SVM EntryAdapter 仍独立选择 exact
Borsh geometry。`Pubkey` 因而可作为一个 32-byte whole-value entry 参数/返回值，RawEntry 与
Keys 两个 consumer 分别验证 round-trip 和 account-projected return；没有 Pubkey-specific
Runtime/Ops/IR/Component/Emit、allocation 或 pointer。详见 [R3-024](tasks/r3-024.md)。
R3-025 再把 program-account close 固定为普通 Lean SDK composition：一次 pre-effect balance
snapshot 后顺序调用已有 `resizeData 0` 和 `transferLamports`，返回同一 snapshot。source/
destination 继续是 compile-time handles，underlying alias/writable/owner/overflow checks 不复制；
...
R3-027 再把 shared enumerable-Set 的 position+1/swap-remove 纯策略分别绑定到 SVM account
RBMap 与既有 EVM static/hashed storage。SVM descriptor 仅由 `(account, baseWord, capacity)`
派生 active value prefix、四个 map header 和七 word compact nodes；显式 initialization、
size/contains/insert/valueAt/remove 支持 value zero，并在 mutation 前验证 count、reverse
position、active backing 与 moved-node evidence。MemberDirectory/UniqueRoster 两个非 Phoenix
consumer 选择 strict 与 idempotent policy；Mollusk 覆盖 full/missing/malformed/short-account
rollback，Surfpool 1.5.0 通过正常 Loader-v3 transaction 部署 exact MemberDirectory ELF。
没有 Set Runtime leaf、Ops/IR/Component/Emit、persistent pointer、heap 或 runtime geometry。
详见 [R3-027](tasks/r3-027.md)。
R3-028 再用两个相邻 typed `Field` 建立 reusable fixed-account discriminator/version header：
`inspect` 完整区分 fresh/current/foreign/unsupported/mixed-zero malformed，`initialize` 先写
version、最后发布 discriminator，exact replay 不写。独立 `Transition` 只允许一个 static
nonzero source→supported target edge，不扫描或在初始化时暗中升级。VersionedLedger 与
VersionedMigrator 选择 strict 与 explicit migration policy；Mollusk 覆盖 short-account 与全部
...
R3 尚未完成；live third+ transient handles（manifest-gated；svm-sdk-004）仍 fail-closed until scratch relayout；
runtime-selected ATA/Memo geometry 与更多 Token-2022 extension semantics 仍待完成
（UTF-8 Memo + one-word `PayloadTransition` 已由 `svm-sdk-006` 收口；resize rent top-up 已由 `svm-sdk-001` 收口；owner reassignment 已由 `svm-sdk-002` 标 n/a 永久 fail-closed）。
...
R4-014 增加 `Context.calldataSize : UInt64`。Environment Component 直接观察
`CALLDATASIZE`，提供 Solidity `msg.data.length` 的无分配标量合同；它不开放 raw calldata、
pointer 或 unchecked byte read，也不增加 top-level Ops/IR/main Emit case。零参数 ABI 调用由
Anvil 精确核对为四个 selector bytes；详见 [R4-014](tasks/r4-014.md)。

R4-015 增加 Cancun `Context.blobBaseFee : UInt256` 与
`Context.blobHash : UInt64 → Bytes32`。Environment Component 分别保持 numeric full-word 与
...
动态角色集合与 indexed Address return 仍 fail closed；fungible 与 bounded ERC-721 asset core
已分别由 R5-006..008/R5-013 完成。
...
R5-013 已完成 bounded ERC-721 core 集成：`Evm.Sdk.Erc721` 在四个 compile-time hashed-map
handles 上组合 owner、per-token approval、operator approval 与 UInt64 balance movement；
Collectible/Badge 独立消费 mint/approve/transfer/burn policy。当前 key contract 只接受 top
...
R5-016 已完成 persistent bounded StorageRing：`Evm.Sdk.StorageRing` 把 shared bounded FIFO
law 绑定到普通 static payload/head/live slots。descriptor 固定正 capacity 和不重叠 geometry；
runtime decision 在任何 literal State update 前拒绝 malformed head/live、noncanonical empty、
full/empty/OOB，wraparound 与 drain-to-empty/clear 都保持 canonical metadata。两个不同 policy
...
R5-018 已完成 persistent bounded StorageCheckpoints：adjacent static UInt64 keys/values vectors
与 live count 组合 strict stored ordering、monotonic append、same-latest overwrite、latest 与
first-key-`≥` lower-bound。V1 descriptor 明确限制 capacity 1..4；malformed count/order 与
zero/unsupported geometry fail closed。EvmCheckpointBook/EvmCheckpointTrace 两个 consumer
保留 literal State vector/count writes，Anvil 通过 test-only storage corruption 验证原子失败；
没有 checkpoint-specific Runtime/Ops/IR/Component/Emit、allocator 或 pointer。详见
[R5-018](tasks/r5-018.md)。

R5-019 已完成 persistent bounded StorageEnumerableMap：existing fixed key vector/live count/
position+1 index 与 distinct key→value `U64Map` 组合 O(1) insert/update/lookup/indexed access/
swap-remove。position namespace 单独证明 presence，使 key/value zero 均可用；middle remove
修复 moved-key position 并清除 removed position/value，last/only remove 只清两份 hashed
payload。EvmConfigMap/EvmScoreMap 两个 consumer 保留 literal State vector/count writes 与
显式 map effects，并用不同 owner/full/malformed-view policy 验证 SDK 与业务边界。descriptor
拒绝 namespace alias，所有 malformed evidence 在 store 前 fail closed；没有 map-specific
...
R5-021 已完成 checked UInt128/UInt256→UInt32 follow-up：shared helper 先检查全部 upper limbs，
再检查 low limb `< 2^32`，之后才执行显式 narrowing。Extract 的 `Except` do-bind continuation
使用既有 `isScalarResult`，不再把 UInt64 当成唯一 fixed scalar；UInt8/16/32/64、Bool 与
registered scalar/newtype 因而共用一个 generic monadic path，而普通 let/state/effect
normalization 保持不变。Accumulator/Config 的 checkpoint/
window 方法分别验证 independent permission、authorization、zero 与 state policy；没有
width-specific Runtime/Ops/IR/CFG/Component/Emit case。详见 [R5-021](tasks/r5-021.md)。

R5-022 已完成 checked UInt128/UInt256→UInt16 follow-up：shared helper 复用同一 allocation-free
policy boundary，在全部 upper limbs 为零后精确要求 low limb `< 2^16`，之后才执行 explicit
narrowing。Accumulator/Config 现有 consumer 分别增加 permissionless batch 与 owner-first
threshold policy，区分 typed width/zero errors，并保持原 UInt64/UInt32 字段和失败交易原子性。
该切片复用 R5-021 的 generic fixed-scalar `Except` bind，不增加 Runtime/Ops/IR/CFG/Component/
Emit、allocation、pointer 或隐藏 state write。UInt8 与 signed casts 继续等待真实 consumer。
详见 [R5-022](tasks/r5-022.md)。

R5-023 已完成 checked UInt128/UInt256→UInt8 follow-up：shared helper 在 upper limbs 全零后精确
要求 low limb `< 2^8`，再执行 explicit byte narrowing，由此补齐 unsigned fixed-scalar
UInt8/16/32/64 targets。Accumulator 的 permissionless mode 路径返回 UInt8；Config 的
owner-first level 路径将成功 byte 显式 widen 为 UInt64 result，以完整保留 `0x1001`
authorization sentinel，避免把控制结果截断为 `1`。两者都在 literal UInt8 state write 前
完成 width/zero policy；没有 Runtime/Ops/IR/CFG/Component/Emit、allocation 或 pointer。
Signed/saturating/wide-to-wide casts 继续 fail closed。详见 [R5-023](tasks/r5-023.md)。

### R6 — 双目标验收

- CI-001 已把 shared Lean guards、SVM build/Mollusk/Surfpool 与 EVM build/Anvil 拆成三条
  无依赖的并行 lane，最终 `test` job 汇总三者；最重的 411-job Lean aggregate 不再在两个
