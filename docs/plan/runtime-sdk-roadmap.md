...
| 层 | 已有 | 主要缺口 |
|---|---|---|
| Shared | target registration、typed extension、Core CFG、bounded scalar frame、checked arithmetic/control、allocation-free UInt128/UInt256→UInt8/UInt16/UInt32/UInt64 SafeCast、allocation-free UInt64 min/max/floor-average/checked-ceilDiv/saturating add-sub-mul/floor-and-ceiling log2-log10-log256/integer-sqrt/full-intermediate floor-and-ceiling-mulDiv、generic fixed-scalar `Except` do-bind、bounded codec schema/resource budget、allocation-free fixed bytes/u128/u256 source values、compiler-erased `BoundedVec` input carrier 与 capacity-preserving operations、bounded Map/Set/Queue/BitSet logical semantics、distinct bounded bytes/UTF-8 string source contracts、aggregate source-schema derivation、target-neutral static projection/rewrite traversal；compiler-owned finite SDK value 可用 representation-free `@[pf_boundary]` 复用 generic static schema/projection/result frame；SVM/EVM scalar、static aggregate、tagged、generic bounded 与 bytes/String input target binding；两边独立的 top-level bounded/tagged output plan；bounded scalar dynamic read；同一 logical schema 的 cross-target plan conformance | signed values/casts、UInt128/UInt256 roots/saturation、fixed point、collection target bindings、bounded mutation writeback、wide/aggregate dynamic element、nested/constructed tagged output、nested dynamic shapes 与更高 resource ceiling |
...
[R1-029](tasks/r1-029.md) 和 [R1-030](tasks/r1-030.md)。
这不表示 R2/R4/R5 已完成。
