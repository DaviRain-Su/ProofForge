...
| contract `STATE` lifecycle | SDK Borsh convention | **one-time init, fail-closed ordinary entries, and versioned schema envelope complete** in wsm-near-init/uninitialized/state-envelope-001; field values remain independent keys | authenticated migration explicit |
...
| private/payable/init | generated entry guards | **explicit private/payable metadata, generated private/non-payable ordering, donation-only payable, repeated-init/uninitialized/schema guards complete**; migration entry absent | entry-adapter policy over context/storage |
...
| N9 lifecycle | **repeated-init, private/payable wrappers, fail-closed entries, and versioned schema envelope done** in wsm-near-init/payable/entry-policy/uninitialized/state-envelope-001; authenticated migration remains | private/deposit/state ordering, migration fixtures |
...
21. **NEAR-STATE-ENVELOPE-1 (wsm-near-state-envelope-001 done):** exact
    ...
    dedicated register and fail closed before state/result reads; authenticated migration remains.
