...
| contract `STATE` lifecycle | SDK Borsh convention | **one-time init marker, legacy field-key guard, and fail-closed ordinary entries complete** in wsm-near-init/uninitialized-001; field values remain independent keys | Near SDK policy; migration/version explicit |
...
| private/payable/init | generated entry guards | **explicit private/payable metadata, generated private/non-payable ordering, donation-only payable, repeated-init and uninitialized-entry guards complete**; version/migration policy absent | entry-adapter policy over context/storage |
...
| N9 lifecycle | **repeated-init, non-payable-by-default, private/explicit payable wrappers, and fail-closed ordinary entries done** in wsm-near-init/payable/entry-policy/uninitialized-001; `STATE` version/migration remain | repeated init, private/deposit/state ordering, migration fixtures |
