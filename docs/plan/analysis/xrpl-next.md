...
13. **XrplMint** — 编译期 minter 才能 mint / `mintTo` / `setCap`；任何人可 `burn`；`halt` / `supp` / `cap` 在 minter 卡上；暂停后 mint/pay/`mintTo`/`burn` 状态码 4。`supp` 随 mint/mintTo 增、burn 减，pay 不变。`cap=0` 不限额。
