...
13. **XrplMint** — 编译期 minter 才能 mint / `mintTo` / `setCap`；编译期 spender 才能 `takeFrom`（源卡是参数三叶，跟 `pay` 一样 `storeOwnerLimbs`；自卡只扣 `allw` 不动 `bal`）；任何人可 `burn` / `approve` / `pay`（halt/supp peek 后切回 caller 再 persist，不把 minter `$bal` 写到别人卡）；`halt` / `supp` / `cap` 在 minter 卡上，`allw` 在授权人卡上；暂停后 mint/pay/`mintTo`/`burn`/`takeFrom`/`setCap`/`approve` 状态码 4。`supp` 随 mint/mintTo 增、burn 减，pay 不变。`cap=0` 不限额；非零 cap 必须 `≥ supp`。不是 Map。
