...
| **Shared bounded/saturating/logarithmic/root/full-precision/fixed-point UInt64 Math（已集成）** | ordinary UInt64 scalar control/arithmetic + bounded local-frame loops | `Core.Math.UInt64` 提供 min/max、overflow-safe floor average、caller-typed checked ceilDiv、explicit saturating add/sub/mul、floor/ceiling log2/log10/log256/sqrt 与 exact-intermediate floor/ceiling `mulDiv`；`Core.FixedPoint.UInt64` 在其上提供 explicit-scale `mulDown`/`mulUp`/`divDown`/`divUp`；BatchSizer/EvmPriceBand 分别绑定 SVM/EVM policy；所有 full-precision/scale paths 复用四个 safe partial products + exact overflow preflight + 同一个 fixed 64-step division kernel；structural guard 钉 loop/frame 并拒绝 additive-loop 误识别和 target extension effect；见 R1-024..031 |
| **Shared bounded bytes/String equality（已集成）** | Core bounded value carriers + existing bounded scalar-frame loop + target codecs | `BoundedBytes.equals` 持有唯一 active-prefix scan，`BoundedString.equals` 通过 compiler-erased byte view 复用；忽略 inactive slots；RawEntry/EvmBounded 分别绑定双 Borsh frame 与双 adjacent ABI tail，String validity 由各 target codec gate；不新增 Runtime/Ops/IR/CFG/Component/Emit、allocation/pointer/shared wire；见 R1-032 |
| **Shared bounded bytes/String ordering（已集成）** | Core bounded value carriers + existing bounded scalar-frame loop + target codecs | typed `LexOrder`/`compareLex?` 与 one-scan `isLexLess` 提供 unsigned first-difference + shorter-prefix policy；String 通过 compiler-erased byte view 复用且不做 normalization/collation；RawEntry/EvmBounded 分别绑定双 Borsh frame 与双 adjacent ABI tail；不新增 Runtime/Ops/IR/CFG/Component/Emit、allocation/pointer/shared wire；见 R1-033 |
| **Shared bounded bytes/String substring search（已集成）** | Core bounded value carriers + static Nat range evaluation + target codecs | `contains` 用单一 static product scan 支持独立 capacities、empty/overlap/malformed fail-closed；String 复用 byte policy；RawEntry/EvmSearch 分别绑定 dual Borsh/ABI inputs；不新增 search-specific Runtime/Ops/IR/CFG/Component/Emit case、allocation/pointer/shared wire；见 R1-034 |
| **Shared bounded bytes/String prefix/suffix（已集成）** | Core bounded value carriers + shared static search kernel + target codecs | `startsWith`/`endsWith` 复用 contains 的 static product scan，只以内核私有 mode 选择首位/末位起点；empty/longer/malformed/inactive-tail policy 一致；RawEntry/EvmSearch 分别绑定 dual Borsh/ABI inputs；不新增 Runtime/Ops/IR/CFG/Component/Extract/Emit case、allocation/pointer/shared wire；见 R1-035 |
| **Shared bounded bytes/String first-position（已集成）** | Core bounded value carriers + shared static search kernel + fixed tagged target codecs | `findIndex?` 用 private position+1 frame 保留 first overlap 并公开 typed `Option UInt64`；String 返回 UTF-8 byte offset；Extract 只顺序化 effect-free bounded scalar loop 并验证 exact Option frame；RawEntry/EvmFindIndex 分别绑定 Borsh/ABI outputs；EVM 统一声明 fixed low-memory `memoryguard` 供 solc stack-to-memory；不新增 feature-specific Runtime/Ops/IR/CFG/Component/Emit case、allocation/pointer/shared wire；见 R1-036 |
