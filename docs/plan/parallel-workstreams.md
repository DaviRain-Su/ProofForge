...
| **Shared bounded UInt64 Math（已集成）** | ordinary UInt64 scalar control/arithmetic | `Core.Math.UInt64` 提供 min/max、overflow-safe floor average 与 caller-typed checked ceilDiv；BatchSizer/EvmPriceBand 分别绑定 SVM/EVM 状态和错误 policy；两个 target SDK facade 均 re-export，且 extraction guard 拒绝任何 target extension effect；见 R1-024 |
