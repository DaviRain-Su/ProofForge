...
| **Shared bounded/saturating UInt64 Math（已集成）** | ordinary UInt64 scalar control/arithmetic | `Core.Math.UInt64` 提供 min/max、overflow-safe floor average、caller-typed checked ceilDiv 与 explicit saturating add/sub/mul；BatchSizer/EvmPriceBand 分别绑定 SVM/EVM policy；structural guard 钉住 saturation preflight/operand 并拒绝 target extension effect；见 R1-024/025 |
