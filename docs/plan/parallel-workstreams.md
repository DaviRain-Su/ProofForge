...
| **SVM-SDK-9 general remainder（ATA slice 已集成）** | SVM-SDK-8、SVM-RT-3 | role-typed static Create/CreateIdempotent/RecoverNested 已直接组合 generic invoke；继续分模块收口 rent-aware resize、runtime-selected/UTF-8 Memo、Token state/program-id policy 与 Token-2022 typed extension facade；未知 extension/runtime account geometry 继续 fail closed；见 R3-010 |
