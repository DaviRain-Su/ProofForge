...
| **Shared bounded/saturating/logarithmic/root/full-precision UInt64 Math（已集成）** | ordinary UInt64 scalar control/arithmetic + bounded local-frame loops | `Core.Math.UInt64` 提供 min/max、overflow-safe floor average、caller-typed checked ceilDiv、explicit saturating add/sub/mul、floor/ceiling log2/log10/log256/sqrt 与 exact-intermediate floor `mulDiv`；BatchSizer/EvmPriceBand 分别绑定 SVM/EVM policy；`mulDiv` 用四个 safe partial products + exact overflow preflight + fixed 64-step division；structural guard 钉 loop/frame 并拒绝 additive-loop 误识别和 target extension effect；见 R1-024..029 |
