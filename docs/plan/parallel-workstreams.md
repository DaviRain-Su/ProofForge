...
| **Shared/EVM-SDK SafeCast（已集成）** | Core wide values、ordinary inline control、generic fixed-scalar `Except` bind | shared UInt128/UInt256→UInt16/UInt32/UInt64 checked narrowing，caller-owned typed error；UInt16/UInt32 路径同时 gate upper limbs 与 exact low-word `< 2^16`/`< 2^32`；ordinary let/state/effect normalization 不变；Accumulator/Config 两 consumer 区分 arithmetic/authorization/zero/write policy；不新增 target Runtime/Ops/IR/CFG/Component/Emit/allocation；见 R5-020..022 |
