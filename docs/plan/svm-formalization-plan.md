...
下一形式化刀：SF-8a FifoCancel 有界折料（`sf-012`，**doing**：纯 L2 cancel 折料骨架已开）。SF-7 Tree 几何 wf（`sf-011`）insert/remove/rotate 已收口；可达/父子互逆操作保持仍可选。
---
...
4. 继续 [sf-012](tasks/sf-012.md)（FifoCancel 折料）；`sf-011` Tree 几何 wf 已收口；按波次向下，每片更新矩阵  
