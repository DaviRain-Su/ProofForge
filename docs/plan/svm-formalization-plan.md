...
下一形式化刀：SF-7 Tree 全树 wf（`sf-011`，**doing**：结构谓词 + paint/value/meta；rotate/insert/remove wf 待补）。EnumerableSet 索引/前缀层（`sf-010`）已收口。
---
...
| OrderedMap / RbTree / Allocator | `Sdk/Storage.lean` | 部分委托 done | Allocator L2 **done**（sf-008）；Map/Tree 模型仍空白 | 对齐 `Examples/Tree.lean` |
...
| SF-7 | Tree 全树 wf 保持 | L2 | **doing** | [sf-011](tasks/sf-011.md) |
...
4. 继续 [sf-011](tasks/sf-011.md)（Tree wf，结构层已开）；`sf-010` EnumerableSet 已收口；按波次向下，每片更新矩阵  
