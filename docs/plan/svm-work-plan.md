...
| 来源 | 变更 | 对本轨影响 |
|---|---|---|
| **Phase 1** Queue 收口 | wrap push/pop + 读回 + peek/initialize/空往返 | **SF-0/1a/1b done**；下一刀 `sf-008`；勿重证已落定理 |
...
Phase 1   证明主线（吃 main 余量）+ L3 阶梯启动 + Runtime 小缺口   ← 立刻
          · A: sf-000..sf-007 **done** → **sf-008 Allocator**（下一刀）
...
| 轨道 | 片 | 状态 |
|---|---|---|
| A | sf-000 … sf-016 | SF-0..SF-4b = **done**；下一刀 SF-5a（sf-008）；其余 todo |
...
1. **主线证明**：SF-0..SF-4b 已收口；下一刀 `sf-008` Allocator  
