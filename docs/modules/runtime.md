...
`Examples/SysSeed.lean` + `runtime-tests/solana/tests/sys_seed.rs`：AllocateWithSeed 给 `create_with_seed(acc0, "vault", program)` 开 16 字节；CreateAccountWithSeed 再转 lamports；缺 signer → `Custom(1)`。
