...
- **部分支持（P5 fixed account profile + official tags 4–11）**：独立 profile 按 canonical
  ...
  withdraw 路径。tags 10/11 CancelMultipleOrdersById：tag 11 free-funds BoundedVec 容量 8（max wire 141）；tag 10 withdraw 仍容量 4（max wire 73；9-account 帧标量局部门槛）
...
- **未支持（P5 remaining instructions/公网）**：完整 placement/matching 策略矩阵、runtime remaining
  accounts、Token-2022 extension 语义、CancelMultiple 超过本片 tag-11 容量 8 / tag-10 容量 4 的 Vec / 满官方容量（tag 10
  四 id withdraw Mollusk 已在容量内覆盖）、全部 Phoenix-v1 指令兼容与公网部署。
