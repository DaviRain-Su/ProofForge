...
`runtime-tests/xrpl/{alphanet-rpc.js,smoke.sh,alphanet-counter.sh,alphanet-init.sh,alphanet-vec.sh}`，不是 `bedrock deploy`。
...
`XrplBalRt`（digest `dd80a5af3243dec2`，`callerBalanceDrops`）。
